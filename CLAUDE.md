...
- Features gated on the EQUIPMENT you hold — **mechanism built, 5 entries authored** (`inventory/equipmentFeatures.js` + `ITEM_SCOPES`). An Items-tab row shows the features that the item is a prerequisite for, collapsed to the feature name. Authored: **Warding Maneuver** (Cavalier L7 — melee weapon or shield), **Savage Attacks**, **Great Weapon Master**'s crit/kill bonus attack, **Eldritch Strike**, 2024 **Remarkable Athlete**'s move. Adding one is a data entry. Un-authored candidates, all the same shape: **Protection** and **Interception** fighting styles (shield / shield-or-weapon reactions), **Shield Master** (shield → bonus shove + DEX-save benefits), **Defensive Duelist** (finesse weapon → +PB AC reaction), **Polearm Master** (polearm → bonus attack + reach OA), **Sentinel** (any weapon → OA riders), **Heavy Armor Master** (heavy armor → damage reduction; needs a `heavyArmor` scope), **Dual Wielder** (two melee weapons — note its +1 AC is already folded into `computeArmorClass`, so only the draw/stow clause is note-worthy).
