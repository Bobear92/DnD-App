...
characters
  ...
  character_data (JSONB),   ← class-specific flexible data (HP, spell slots, features, skill profs, etc.)
                             ...
                             ←   active_effects (string[]): the keys of the effects the character has switched ON (see effects/activeEffects.js). Currently only `giants_might`. A long rest resets the feature's CHARGE but never clears this — a 1-minute effect ended long before the rest, and clearing it would imply a duration the app doesn't model
...
- **Active effects (a feature you switch ON and OFF) — mechanism built, 1 entry authored** (`effects/activeEffects.js` + `ActiveEffectsBanner`). **This deliberately REVERSES the app's long-standing "no active-effect model" line** — the line that still keeps Rage damage, Divine Smite and spell-granted magic weapons as prose, because a bare "+2d6" on a card would be false most of the time. Rune Knight **Giant's Might** forced it: the feature changes the character's SIZE, grants advantage on two roll types and adds a scaling damage die, which is far too much to leave as a paragraph the reader applies by hand. State is `character_data.active_effects: string[]`; a definition is a data entry gated by class/subclass/level/edition, whose `grants(level)` returns only the fields it changes (`size`, `advantageAbilities`, `advantageSaves`, `attackDie`, `reachBonus`) and whose `summary(level)` is the banner's text — so no surface retypes a number the definition owns. **Duration is deliberately NOT modelled** (nothing tracks rounds): an effect runs until switched off, and the stated duration is text. Two surfaces: the **Action Economy card that starts it** carries the toggle (Use spends a charge AND switches the effect on in ONE patch — a counter that went down without the effect coming on is a trap; End never refunds), and the **`ActiveEffectsBanner` above the tabs** on CharacterDetail shows every running effect with its numbers and an End control. The banner sits outside the tabs because one effect changes numbers on several of them, and it is wired CENTRALLY (like the Defenses card, not a sheet slot) so hand-written sheets get it too. It is also the only place creature **size** is ever displayed — `creatureSize` was previously computed for the Heavy-weapon rule and shown nowhere, so growing Large was invisible. `creatureSize(characterData, race, ctx)` now takes an optional effect context and resolves the grown size at that ONE function every consumer already calls, so the Items and Action Economy tabs can't disagree. A long rest resets the *charge* but deliberately does NOT switch the effect off (a 1-minute effect ended long before the rest; a rest clearing it would imply a duration the app doesn't model). Adding Rage/Hex/Hunter's Mark should be a data entry plus one consumer — check whether `grants` needs another field, not whether it needs a sibling table.
