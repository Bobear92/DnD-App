...
│   │   ├── WizardSheet.jsx      # Wizard (5e): full caster, spellbook, arcane recovery; Features tab renders PortentTracker under the subclass panel (Divination only); Spells tab: "Prepared" sub-tab (Arcane Recovery "Use (Short Rest)" button — disabled when no recoverable expended slots, on click opens a confirmation dialog ("This can only be used once per short rest.") with Cancel / Use Recovery buttons — only on Confirm does it restore expended slots highest-value-first up to ⌈level/2⌉ slot-levels (levels 1–5 only, never 6th+) and mark itself Used; clicking "Used" resets it + slot tracker (− for all, + GM-only) + Cast button per prepared spell + cantrips) + "Prepare Spells" sub-tab (spellbook chips + lock/unlock + Other Prepared + encyclopedia link) ✅
