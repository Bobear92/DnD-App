...
│   │   #   subclass/    → SubclassDetails, SubclassOverview, SubclassPickerWithDetail, BattleMasterPanel, PortentTracker, DraconicAncestorPicker, draconicData, subclassCombatNotes
...
│   │   #   inventory/   → InventoryTab, ItemPickerDialog, WalletCard, WeaponPropertyBadges, StartingEquipmentStep, ammunitionData, currencyData, inventoryData, inventoryProficiencies, startingEquipmentData, startingEquipmentResolver, toolsData, weaponPropertyData
│   │   #   spells/      → ClassSpellBrowser, SpellLevelTabs, SpellList
│   │   #   race/        → TraitBadge, RacialResourceTracker, raceProficienciesData, raceTraitsData, racialRestResources, backgroundSkillsData
...
│   │   │   ├── CasterSpellBlock.jsx  # Creation spell pickers + Prepared/Prepare sub-tabs + Arcane Recovery + spellbook chips + lock/unlock (uses useSlotCaster); converged single prepare UI for both Wizard editions. **Also a `kind:'known'` mode** (subclass casters — Eldritch Knight): no prepare flow/lock/Arcane Recovery — renders the caster note + the shared slot-tracker grid + editable Cantrips Known / Spells Known SpellLists with Cast buttons (`known-caster-block`; spells are picked at level-up via the New Spells step, the lists allow swap edits); returns null during creation (subclass casters unlock after L1)
│   │   │   ├── RestResourceTracker.jsx # Use-button + confirm ("won't come back until a short/long rest") + − recover for rest-rechargeable features (Second Wind/Action Surge/Indomitable/Memorize Spell); each row shows a short "what it does" line under the label from the config entry's optional `description` (e.g. Second Wind → "Bonus action: regain 1d10 + your Fighter level HP."; passed through useRestResource; data-testid rest-resource-desc-{key}); data-testid rest-resource-{key}, rest-use-confirm-button. Exports `RestResourceControl` (the per-resource −/Use + confirm-dialog control, `idPrefix` namespaces the confirm-button test id) so other views (Action Economy tab) reuse the same Use mechanic on a single resource
...
│   │   ├── subclassCasterData.js # Subclass-granted spellcasting (casters by SUBCLASS, not class): SUBCLASS_CASTERS[class][edition][subclass] + getSubclassCaster(class, edition, subclass, level?) (level-gates on the entry's unlockLevel when level passed). **Eldritch Knight (both editions)**: `{kind:'known', spellcastingAbility:'intelligence', unlockLevel:3, slotsForLevel (THIRD_CASTER_SLOTS — third-caster table, 9-wide, slots L3→4th-level @L19), cantripsKnownAt (2→3@10), spellsKnownAt (3@3…13@20), note}`. 2024 EK words its list as "prepared" but is mechanically the same fixed list swapped on level-up, so both editions are modeled as KNOWN casters (like the app's 2024 Sorcerer/Bard). Consumers: ClassSheet (activeCaster), CasterSpellBlock kind:'known', LevelUpWizard New Spells targets, CharacterDetail hasSpells, backend rest. Arcane Trickster = the next data entry. Tests: subclassCasterData.test.js (9) ✅
