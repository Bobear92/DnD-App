...
├── characters/
│   ├── characterService.js      # API client: CRUD, toggleVisibility, deleteCharacter, uploadImage, deleteImage, getTimelineEvents, createTimelineEvent, removeTimelineEvent, getCharacterNpcs, createCharacterNpc, removeCharacterNpc, applyRest(campaignId, restType, characterIds); exports mapCharacterImageUrl(path)
...
│   │   ├── WizardSheet.jsx      # Wizard (5e): full caster, spellbook, arcane recovery; Features tab renders PortentTracker under the subclass panel (Divination only); Spells tab: "Prepared" sub-tab (Arcane Recovery "Use (Short Rest)" button — disabled when no recoverable expended slots, on click actually restores expended slots highest-value-first up to ⌈level/2⌉ slot-levels (levels 1–5 only, never 6th+) and marks itself Used; clicking "Used" resets it + slot tracker (− for all, + GM-only) + Cast button per prepared spell + cantrips) + "Prepare Spells" sub-tab (spellbook chips + lock/unlock + Other Prepared + encyclopedia link) ✅
...
│   │   ├── RacialResourceTracker.jsx # Use-counter widget for rest-rechargeable racial traits; renders null when no applicable resources; reads/writes <key>_used counters in character_data; +/− buttons hidden when readOnly; rendered in CharacterDetail Stats tab "Racial Features" card ✅
...
│   │       ├── WizardSheet.jsx    # Wizard (2024): memorize spell L1, arcane recovery; Features tab renders PortentTracker under the subclass panel (Diviner only); Spells tab: "Prepared" sub-tab (Arcane Recovery "Use (Short Rest)" button — disabled when no recoverable expended slots, on click actually restores expended slots highest-value-first up to ⌈level/2⌉ slot-levels (levels 1–5 only, never 6th+) and marks itself Used; clicking "Used" resets it + slot tracker (− for all, + GM-only) + Cast button per prepared spell + cantrips) + "Prepare Spells" sub-tab (spellbook chips + lock/unlock + Other Prepared + encyclopedia link) ✅
...
│   │   └── RacialResourceTracker.test.jsx # getRacialRestResources (none/Breath Weapon/Drow level-gating/Infernal); tracker null when no resources; Relentless Endurance render + used count; + expends / − recovers; Breath Weapon short-or-long-rest label; readOnly hides buttons (12 tests)
...
│   └── pages/
│       ├── EncyclopediaPage.jsx      # 3-tab layout: Classes (two-panel browser, edition toggle) | Skills (static skill browser) | Spells (search+school+level filters, GM override; for GM contains two sub-tabs inside the Spells tab — "System Spells" default, "Campaign Spells" GM-only — players see only the system list with no sub-tab bar); GM + player ✅
│       ├── encyclopediaService.js   # Axios spell API client: getSpells(campaignId), getSpell(id), createSpell, updateSpell, deleteSpell
│       ├── EncyclopediaPage.test.jsx # header/Classes tab, empty state, all 13 classes (including Artificer), edition toggle defaults (5e/2024), classService call args, overview renders, empty state hides, edition switch re-fetches, player access, loading state; Spells tab renders, clicking tab shows SpellsTab, edition toggle hidden on Spells tab; Campaign Spells is no longer a top-level tab, System/Campaign sub-tabs render inside Spells for GM only, player sees no sub-tabs, clicking Campaign Spells sub-tab swaps for CampaignSpellsTab, Spells sub-tab defaults to System Spells; Skills tab renders, player access, click shows SkillsTab, edition toggle hidden on Skills tab (24 tests)
...
| `/campaigns/:campaignId/encyclopedia` | EncyclopediaPage | ✅ Functional (Classes tab: edition toggle + class browser; Skills tab: 18 static 5e skills + ability filter; Spells tab: search/school/level filters + GM override; Spells tab contains GM-only sub-tabs — System Spells / Campaign Spells) |
...
- **CharacterDetail — race-granted skills on skill panel:** `SkillsDisplay` computes `raceGranted = getRaceGrantedSkillsFromTraits(character_data.race_traits)` and unions it with stored `skill_proficiencies`, so existing characters (whose stored array may not include race grants) still show Perception/Intimidation proficient. Race-granted skills render with an **emerald** indicator; class/background proficiency uses `bg-primary` (gold, `#d4af37`); expertise uses purple. Legend reads `"Purple = expertise · Gold = proficient · Emerald = from race"` (last clause appears only when race grants apply). (The legend says "Gold" because the proficient swatch is `bg-primary` = gold, not blue.)
...
- **CharacterDetail — tab layout:** five-tab structure for spellcasters, four-tab for non-spellcasters (shadcn/ui Tabs); default tab is "narrative":
  ...
  - **Tab 1 "Stats"** — Identity & Ability Scores card + Hit Points & Movement card (class sheet with `section="stats"`) + **Racial Features card** (`RacialResourceTracker`, shown only when the character has ≥1 rest-rechargeable racial trait — Breath Weapon, Relentless Endurance, Drow Magic L3+, Infernal Legacy L3+; shares `classSection`/`saveClassData` so Save/Reset persist with the rest of character_data)
...
- Loot table UI
- Encyclopedia browsing UI — Classes, Skills, and Spells tabs exist; Monsters, Items tabs not yet built
...
├── test_characters.py              # character CRUD + visibility, TestGmNotes (gm_can_set/get, stripped_from_owner, player_cannot_set), TestGmDelete (gm_can_delete, other_player_cannot), TestCampaignEdition (defaults_to_5e, create/update/list), TestCharacterListFieldRoundTrip incl. character_data, TestApplyRest (GM can short/long rest, player 403, non-member 403, cross-campaign filter, Warlock pact slots, Wizard HP+slots, hit dice recovery, response shape, Dragonborn Breath Weapon short-rest reset, Relentless Endurance long-rest-only, Drow darkness level-gated, Portent cleared on long rest, Portent untouched on short rest) (48 tests)
