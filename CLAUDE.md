...
- Features gated on the EQUIPMENT you hold — **mechanism built, 6 entries authored** (`inventory/equipmentFeatures.js` + `ITEM_SCOPES`). An Items-tab row shows the features that the item is a prerequisite for, collapsed to the feature name. Authored: **Warding Maneuver** (Cavalier L7 — melee weapon or shield), **Savage Attacks**, **Great Weapon Master**'s crit/kill bonus attack, **Eldritch Strike**, 2024 **Remarkable Athlete**'s move, **Heavy Armor Master** (the `heavyArmor` scope — deliberately narrower than `bodyArmor`, since the feat does nothing in light/medium and showing it there would state a benefit the wearer lacks; its text is built from the same `damage_reduction` feat effect the Defenses panel reads, so the number can't drift between the two surfaces). Adding one is a data entry. Un-authored candidates, all the same shape: **Protection** and **Interception** fighting styles (shield / shield-or-weapon reactions), **Shield Master** (shield → bonus shove + DEX-save benefits), **Defensive Duelist** (finesse weapon → +PB AC reaction), **Polearm Master** (polearm → bonus attack + reach OA), **Sentinel** (any weapon → OA riders), **Dual Wielder** (two melee weapons — note its +1 AC is already folded into `computeArmorClass`, so only the draw/stow clause is note-worthy).
