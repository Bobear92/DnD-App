...
- **Rune Knight (Fighter, 5e only — the 2024 PHB ships no Rune Knight) — IN PROGRESS.** Built: **Giant's Might** (bonus-action card + PB/long-rest pool + the active-effect toggle + the scaling damage rider + the grown size) and **Runic Shield** (reaction card + PB/long-rest pool), both reset by the backend long rest and mirrored in `getRestSummary`. Still to build, in order: **Bonus Proficiencies** (smith's tools + Giant — a FIXED grant, which has no mechanism yet; planned as a derived `classData/subclassProficiencies.js`, the sibling of `subclassSpells.js`, since there is no pick to store), **Rune Carving** (a `levelChoicesData` pool of 6 runes, 2→3→4→5 known, Hill gated at L7 and Storm at L15, swap-one-on-level-up, with the option data in a `runesData.js`), **rune → item assignment** (the third instance of the designation pattern behind `bonded_weapon_uids`/`hex_weapon_uid`; one rune per object, cleared on a long rest), **Channel Rune** (six `hidden(ctx)` rest rows, one per rune the character knows, 1/short-or-long rest → 2 from **Master of Runes** at L15), and the **rune passives** (skill advantage → needs the `SKILL_ADVANTAGE_SOURCES` extraction, since that is currently hardcoded to Champion in `remarkableAthlete()`; Hill Rune's poison resistance → `defenses.js`). **Deliberate scope limit on rune-bearing objects:** RAW a rune may be inscribed on "a weapon, a suit of armor, a shield, a piece of jewelry, or something else you can wear or hold in a hand", but the assignment will offer **weapons, armor and shields only** — the only wearable/holdable things the inventory models as equippable. **Revisit jewelry and other worn objects when items/armor get more robust** (they live in `adventuring_gear`/`magic_items` today with no worn state).
