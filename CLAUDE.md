...
Campaign PUT accepts all settings fields: `use_alignment`, `ability_score_method`, `allow_reroll_ones`, `leveling_type`, `currency_type`, `starting_equipment`. `currency_type` ("standard" | "full") and `starting_equipment` ("equipment" | "equipment_or_gold" | "none") are in `CampaignResponse` + `CampaignListItem` so they reach the frontend CampaignContext.
...
│   ├── pages/CampaignSettingsPage.test.jsx # currency setting (GM sees currency-type-select, player read-only "CP, SP, GP, PP" / "CP, SP, EP, GP, PP") + starting-equipment setting (GM sees starting-equipment-select, player read-only "None") (5 tests)
...
- **CharacterDetail — tab layout:** six-tab structure for spellcasters, five-tab for non-spellcasters (shadcn/ui Tabs); default tab is "narrative":
  ...
  - **Tab 2 "Features"** — `{char_class} Features` card (class sheet with `section="features"`, includes subclass picker/lock, class features up to current level). For the data-driven sheets (Fighter/Wizard) the content splits into two button sub-tabs — "General {ClassName} Features" and "{Subclass} Features" (named after the chosen subclass); subclass picker/details + Portent live under the subclass sub-tab
...
- **LevelUpWizard (dynamic-step modal):**
  - Steps: HP → (Subclass, conditional) → Features → (Ability Scores, ASI levels) → (New Spells, known casters) → (Proficiencies, conditional) → (Maneuvers, Battle Master learn levels) → Confirm
  ...
  - **Ability Score Improvement step** appears whenever the new level's features include "Ability Score Improvement" (detected from the feature tables — class-agnostic; 4/8/12/16/19, +6/14 Fighter, +10 Rogue). Allocates a 2-point budget (+2 to one or +1 to two, each capped at 20) via per-ability ± steppers; Next blocked until both points spent. The increases update **top-level** character ability fields (not character_data), passed to `onComplete(newLevel, newCharacterData, extraUpdates)` as a 3rd arg **only when non-empty** (so existing 2-arg callers/tests are unaffected); `handleLevelUpComplete` spreads `extraUpdates` into the character PUT + identity draft. `data-testid` asi-row-{ability}, asi-inc-{ability}, asi-dec-{ability}, asi-remaining. (Taking a feat instead of the ASI is not yet built — see "Frontend Not Yet Built".)
...
- Feat selection UI — partially built: feats live in the encyclopedia (Feats tab, edition-aware) and the Variant Human creation flow picks one feat + skill (stored in `character_data.feats`). The LevelUpWizard ASI step now handles the **ability-score** half of an ASI level; **"take a feat instead of the ASI"** is still deferred (would reuse FeatPicker + featService + prereq gating, appending to `character_data.feats`). The class-sheet feat section is also still deferred.
