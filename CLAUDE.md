...
### Prose-only needs sign-off (never self-assigned)
**When a feature, feat clause or pool option is triaged as prose-only / narrative / a `note`, FLAG IT AND SHOW THE USER before calling the work complete.** Collect them into one short list — one line each: `feature › clause — what it would take, and the MISSING MODEL that stops it` (no concentration/duration state, no distance-to-target, no surprise state, no target at all, no computed tool-check number, no mount entity, a choice the app never persists). Say which you think are genuinely un-modelable vs. merely **deferred**, and let the user decide.

"It's flavor" / "it's narrative" / "can't be computed" with nothing named is NOT a reason — if you can't name the missing model, the clause is probably mechanizable and the surface just hasn't been found. This is the escape hatch that hides missed mechanics: Shield Master's Interpose Shield, Sentinel's and Polearm Master's opportunity-attack clauses, and Storm Rune's entire reaction all shipped inside prose and were all found by the user in QA. Enforced in `/subclass-features` (Phase 1.5), `/feat-effects` and `/class-config`.
...
characters
  ...
  character_data (JSONB),   ← class-specific flexible data (HP, spell slots, features, skill profs, etc.)
                             ...
                             ←   active_effects (string[]): the keys of the effects the character has switched ON (see effects/activeEffects.js). `giants_might` and `channel_rune_frost` (Frost Rune's Channel Rune — 10 minutes of +2 to Strength/Constitution checks and saves; the first effect gated on EQUIPMENT as well as level, so it simply stops resolving if the bearing item is unequipped mid-effect rather than being force-cleared). A long rest resets the feature's CHARGE but never clears this — a 1-minute effect ended long before the rest, and clearing it would imply a duration the app doesn't model
                             ←   runes (string[]): Rune Knight Rune Carving (5e Fighter, L3+) — the runes the character knows (2/3/4/5 at levels 3/7/10/15), chosen in the LevelUpWizard `level-choices` step from the `levelChoicesData` pool in `runesData.js` and shown read-only (with an owed-slot picker) by the sheet's `KnownOptionsBlock`. Hill is gated at L7 and Storm at L15 via option `minLevel`; one rune may be swapped per Fighter level. The rune save DC is 8 + PB + CON — CONSTITUTION, not the Intelligence Arcane Shot uses — and rides on the choice's `derived` line. Which OBJECT each rune is carved on lives in `rune_items` (below)
                             ←   rune_items ({[runeName]: uid}): Rune Knight Rune Carving — which inventory entry each known rune is carved on. Keyed by RUNE (the scarce resource: you have 2-5 runes and unbounded objects), which makes "one rune per object" a uniqueness check over the values. Assigned from the ITEM CARD in the Items tab (weapons/armor/shields only — the only wearable/holdable things the inventory models as equippable; jewelry waits on a worn-item model). This is the third instance of the designate-an-item pattern but deliberately NOT built on `WeaponDesignationPanel`: that one is ONE feature -> N weapons with the picker in a feature panel, this is N runes -> N items 1:1 with the control on the row. **Knowing a rune grants nothing** — every consumer gates on `activeRunes()` (known + carved + the bearing item EQUIPPED), so unequipping the axe removes the passive and its Channel Rune card. Deliberately NOT cleared on a long rest: RAW you re-inscribe then, but the app has no "already inscribed today" state and wiping five assignments each morning is worse than the divergence. See inventory/runeCarving.js
                             ←   channel_rune_<rune>_used (integer, x6): Rune Knight Channel Rune — one counter PER RUNE (cloud/fire/frost/stone/hill/storm), because RAW recharges each rune independently. 1 use, or 2 from Master of Runes (L15). Reset by `_compute_rest_patch` on BOTH rest types (the only Rune Knight pool that returns on a short rest). Its rest row + Action Economy card each `hidden()` themselves unless the rune is live
  ...
  experience_points (integer, default 0),   ← XP total; used when leveling_type="experience".
                                            CLAMPED to [0, 355_000] on create + update by the service
                                            (MAX_EXPERIENCE_POINTS) — 355,000 is the level-20 threshold and
                                            XP past it buys nothing, so an over-award tops out instead of
                                            running away. Enforced in the SERVICE, not just the UI, so it
                                            holds for any caller; the frontend mirrors it as MAX_XP in
                                            CharacterDetail.jsx so the GM never sees a number the save changes
  level_up_pending (boolean, default false), ← set true when XP threshold crossed or GM triggers milestone LU.
                                            RE-DERIVED (not cleared) when a level-up completes, via
                                            CharacterDetail's `qualifiesForLevelUp(level, xp)`: one award can
                                            bank several levels, and clearing it outright stranded the character
                                            at the intermediate level until another point of XP made the
                                            add-XP path recompute the flag. Milestone campaigns still clear it —
                                            there the flag is GM-triggered and XP means nothing.
  ...
  appearance (JSONB, nullable),                    ← the character's PHYSICAL DESCRIPTION — free-text, display-only,
                                                     visible to every campaign member like `backstory` (no stripping).
                                                     NPCs have had age/gender/height/weight/appearance since that module
                                                     was built and player characters had NONE of it, so a GM could describe
                                                     a tavern keeper but not the party's own fighter. ONE JSONB column
                                                     rather than ~20 nullable String columns (the NPC shape): nothing
                                                     computes from any of it and the field list is MEANT to grow, which
                                                     would otherwise be a migration each time — the catalog lives in the
                                                     frontend (`appearance/appearanceData.js`, 23 fields in 4 groups) and
                                                     adding one is frontend-only. Deliberately NOT part of `character_data`,
                                                     which is class-specific MECHANICAL state. **Nothing may ever compute
                                                     from it** — `height` in particular must not reach `creatureSize`:
                                                     Great Stature's +3d4 inches never crosses a size category, and a sheet
                                                     where a tall character grappled differently would be inventing a rule.
                                                     Empty strings are stripped client-side (`cleanAppearance`) so an
                                                     untouched field never persists as "". Present in CharacterCreate/
                                                     Update/Response, deliberately ABSENT from CharacterListItem (the list
                                                     view is lightweight and no card shows a description)
...
feats
  ...
  effects (JSON, nullable),  ← structured mechanical effects so a feat is more than a description card; array of typed objects
                             ...
                             ←   | {kind:"ability_score", ability, amount} (fixed) | {kind:"attack_mod", target:"unarmed", dice} | {kind:"action", name, economy, description, trigger} | {kind:"spell_grant", source_kind, cantrips, leveled, fixed, free_cast, ability} (Magic Initiate — grants picked spells) | {kind:"fighting_style", style} (a 2024 Fighting Style feat — Archery/Defense/Dueling/Thrown Weapon Fighting/Two-Weapon Fighting — merged into fightingStyles.gatherFightingStyles so it reaches the same attack/AC math as a class-granted style; before this those feats did nothing) | {kind:"maneuver_grant", count, die} (Martial Adept — player picks N Battle Master maneuvers + a die; snapshotted to choices.maneuvers; for a Battle Master the die/maneuvers fold into the shared Combat Superiority pool, else a standalone d6 in the Feats tab) | {kind:"note", text}
  ...
  Seeded via seed_feats.py: 41 PHB-2014 feats (edition "5e") + 73 PHB-2024 feats (edition "5.5e",
    ...
    Re-running REFRESHES an existing system feat's `description`, `prerequisites` and `effects` from this file (it is the source of truth), and then **`sync_character_feat_snapshots` refreshes the `effects` copied onto every character's feats** (matched by feat id; `choices` untouched) — so a feat mechanized later reaches characters who already had it with no remove + re-add. `tests/test_feat_seed_data.py` GUARDS the authored data: a feat whose text grants a reaction must author a `reaction` action, and an Attack-action bonus action must author a bonus action with the Attack-action trigger; so a mis-authored feat is corrected by editing here and re-seeding — this is how the **2024 Sharpshooter** text was fixed: it had been seeded as the 2014 clauses with Great Weapon Master's "+proficiency bonus to damage" pasted on, and is really Bypass Cover / Firing in Melee / Long Shot (+30 ft normal range) with NO damage bonus.
    Also backfills `effects` from FEAT_EFFECTS_5E (5e: 23/41 complete) + FEAT_EFFECTS_2024 (5.5e: 29/73 complete), authored per their own edition's rules (2024 Alert/Observant differ from 2014, Origin feats grant no ASI, etc.). The rest are honest prose-only `note`s — pure rules-text riders, most fighting-style riders, Crafter/Musician sub-pools, or Epic Boons.
    **Coverage is CLAUSE-level** (it used to be feat-level, which let Grappler pass on its +1 ability score while its headline clause sat as text — found in QA). Every `note` carries one of three statuses via `_note(text, surfaced=None, unmodelable=None)`: **pending** (no tag — not built; the worklist), **`surfaced="<where on the sheet>"`** (built by a frontend table/card keyed on the feat name — the note only restates it), or **`unmodelable="<missing model>"`** (the USER signed off; never self-assigned). A feat is **complete** only with no pending clause. `python report_feat_effects.py` prints every pending clause per edition. **`--check` is a RATCHET GATE** on the committed `backend/feat_coverage_baseline.json` — complete feats may not DROP and pending clauses may not RISE (so a new text-only note fails CI until built, tagged `surfaced`, or signed off) — currently 5e 23/41 complete + 18 pending, 5.5e 29/73 complete + 44 pending. Pure logic unit-tested in `tests/test_report_feat_effects.py`; runs in CI + `/ship` Step 3b (not pytest, since it reads the seeded dev DB). Ratchet with `--write-baseline` after building more. Authoring procedure: the `/feat-effects` skill.
...
weapons
  id, name, damage, damage_type, weight, cost, weapon_category, weapon_type,
  properties (Text: comma-joined property NAMES, e.g. "Ammunition, Heavy, Two-Handed"),
  range_normal (Integer, nullable), range_long (Integer, nullable),
                             ← the distance band: attacks up to `range_normal` are unmodified,
                               beyond it but within `range_long` are at disadvantage. TWO INTEGERS,
                               not a "150/600" string, so no consumer parses prose (the mistake the
                               spells table's free-text `range` made — see docs/spell-upcast-review.md).
                               NULL for a pure melee weapon: its 5 ft is REACH, a different concept
                               (a Reach weapon's is 10). A THROWN melee weapon DOES get a band — its
                               throw range — so a dagger reads "Thrown 20/60 ft". `range_long` may be
                               NULL alone (one distance, no falloff — a net).
                               Seeded from the SRD `range` (ranged weapons) / `throw_range` (thrown);
                               seed_items.py previously DISCARDED both. 16 of 38 system weapons carry
                               a band (9 ranged + 7 thrown); the other 22 are melee and correctly NULL.
                               `backfill_weapon_ranges()` fills rows that predate the columns, since
                               the create path skips existing weapons (same shape as seed_feats.py
                               backfilling `effects`); it never nulls a value a GM set.
  description, owner_type (ENUM), owner_id, created_at, updated_at
...
| PUT | /api/characters/{id} | Yes (owner or GM); accepts `appearance` (the physical-description JSONB; a write REPLACES the object, which is what lets a cleared field clear); uses `CharacterGmUpdate` — only GM can set `gm_notes`/`is_visible_to_players`; GM cannot overwrite `personal_notes`; `experience_points` is clamped to [0, 355_000] |
...
| `/campaigns/:campaignId/encyclopedia/mechanics/damage-mitigation` | DamageMitigationPage | ✅ Functional (static "Taking Damage" reference — Flow B, the math lives in `defenses/defenses.js`: the four ways damage changes (resistance/immunity/vulnerability/flat reduction), **the RAW order of operations** (modifiers → vulnerability doubles → resistance halves → flat reduction subtracts) worked through the exported `applyDamage()` with the common wrong order shown for contrast, resistance not stacking + cancelling against vulnerability, rounding down, temporary hit points (never stack, not healing, don't raise max, vanish on a long rest) with the 7 real in-app sources and WHO each grants to, reaction-based reductions (Parry / Protective Field / Deflect Missiles / Song of Defense / Warding Maneuver) listed separately with why the Defenses card excludes them, damage-vs-condition immunity, and an honest 'what the app can't track' (nothing gives a CHARACTER vulnerability — it only appears outward-facing; damage isn't rolled; temp HP is a plain number; conditional defenses aren't switched on). No edition toggle — the rules are identical in both. Cross-links Magical Attacks rather than restating "nonmagical"; linked from the Defenses card via `defenses-learn-more`) |
...
| `/campaigns/:campaignId/encyclopedia/mechanics/special-attacks` | SpecialAttacksPage | ✅ Functional (static "Special Melee Attacks" reference — Grapple and Shove, Flow B off the new `combat/specialAttacksData.js`, which also builds the sheet's cards so the two cannot drift. **Edition toggle**, because the editions genuinely differ in how the attempt resolves: 2014 is a contested Strength (Athletics) check, 2024 is a Strength/Dexterity save against your Unarmed Strike DC (8 + STR + PB). Leads with the misreading it exists to correct — neither is an action of its own; each REPLACES one attack of the Attack action, which is why Extra Attack makes a grapple nearly free. Covers the size limit worked through `sizeLimitNote` for Small/Medium/Large, what Grappled and Prone actually do, escaping (the target's whole action), when to reach for one, an at-the-table example run through the helper, the in-app feats/features that change them per edition (Grappler, Shield Master, Tavern Brawler, Unarmed Fighting, 5e's Rune Knight pair + Monster Slayer, 2024's Crusher), and an honest "what the app doesn't track" (no grappled/prone state, no target, no rolled contest). Linked from the Grapple and Shove cards in the Action Economy tab) |
...
- Action Economy tab — **framework + Fighter (5e + 2024) built** (vertical slice). **RULE: "…you can use your reaction …" means a card in the REACTIONS tab** (feats: author a `reaction` action effect — Shield Master's Interpose Shield was a prose note, found in QA; curated class/subclass features: a guard test fails any mapped feature whose text grants a reaction but has no `tab:'reaction'` entry. **That guard reads SUBCLASS_DATA feature descriptions ONLY, which is a real blind spot** — a POOL OPTION's rules text lives in its own module, so Rune Carving passed it while saying nothing but "a Channel Rune effect" and the per-rune reaction prose sat unscanned in `runesData` (found in QA by a player reading the rune list, not by the suite). Its **option-pool sibling guard** closes that: it scans each option's PLAYER-FACING text — including the parenthetical cost, "Channel Rune (reaction, …)", since a pool states the cost as structured data and the sentence never says "use your reaction" — and checks BEHAVIOURALLY that a reaction card appears for a character actually holding that option. Pools the tab does not cover yet are NAMED in its PENDING list (Battle Master Parry + Riposte, both editions), never silently skipped. Both guards exclude "its Reaction": Commander's Strike spends an ALLY's, which is no card on this sheet. **When a feature's real text lives outside the table a guard reads, the guard does not cover it** — check that before trusting a green suite on a new pool). **RULE: "If you take the Attack action, you can use a bonus action …" is an Action + Bonus COMBO, never a lone bonus action** — recognise it from the text. Feat `action` effects are routed automatically by `isAttackActionBonus` (+ per-feat `ATTACK_ACTION_COMBO_GATES`: Polearm Master's opener weapon, Shield Master's shield warning); curated class/subclass entries are guarded by a test that fails any `tab:'bonus'` entry with that wording. Un-wired instances waiting on their class: War Priest, Sudden Strike, Monk Martial Arts/Flurry/Searing Arc Strike, Commander's Strike. Auto-derived sources (weapon attacks, spells-by-casting-time, Two-Weapon Fighting, universal action menu) work for ALL classes; the curated class-feature→action-economy map (`actionEconomyData.js`) currently covers Fighter only. **Subclass features now have their own map** (`SUBCLASS_FEATURE_ACTIONS_*`, level-gated from SUBCLASS_DATA — wired: Eldritch Knight Weapon Bond + the War Magic combo + Arcane Charge rider as curated blocks; Arcane Archer Arcane Shot + Curving Shot; Cavalier Unwavering Mark (split across two surfaces, below) + Warding Maneuver (reaction) + Ferocious Charger; Echo Knight Manifest Echo (bonus, computed echo AC) + Unleash Incarnation (**attached to the melee attack cards**, see below) + Echo Avatar + Shadow Martyr + Reclaim Potential (computed temp HP); Psi Warrior's six powers, below; **Rune Knight** Giant's Might — the first entry carrying an **`activeEffect`** key, so its card renders the active-effect toggle instead of the plain Use control — and Runic Shield).  **A subclass feature name may map to a SINGLE entry or an ARRAY of them** — some features bundle several powers at different costs under one name, and one card would have to pick a single cost badge and misstate the rest. The Psi Warrior drove it: `Psionic Power` is a reaction (Protective Field), an action (Telekinetic Movement) and an attack rider (Psionic Strike) at once, and `Telekinetic Adept` is a bonus action (Psi-Powered Leap) plus a second rider (Telekinetic Thrust). Array members carry their own `name`; a single entry still takes the feature name or its `displayName`. Its pool also gets a read-only **`PsionicEnergyPanel`** in the Features tab (Fighter config `subclassPanels['Psi Warrior']`, the same slot Battle Master and Eldritch Knight use): the pool DEFINITION — `{total} × {die}`, "twice your proficiency bonus", the next die-growth level in words via `nextPsionicDieStep`, an echoed unspent count, and the strike/save-DC rolls that fold in INT. Deliberately has no Use controls: spending already lives on the rest tracker and the action cards, and a third mutation point for one counter is how surfaces drift. Psi Warrior is also the app's first **one pool, many consumers** resource — five entries share one `psionic_energy_used` key while the three free-once-per-rest powers each carry their own small charge key — and its numbers come from `psiWarriorData.js`, never the stored feature blurbs, which are wrong in three places (a flat "d6" instead of the d6→d12 scaling, "fly speed equal to" instead of twice walking speed, and Telekinetic Movement called a bonus action when RAW is an action). It also drove the two ways one Use control can touch a SECOND resource row — deliberately separate fields, since they are opposite operations: **`fallbackResourceKey`** (entry) / **`fallbackKey`** (class-config row) = FREE-USE-THEN-PAY, where Use spends the row's own charge while it lasts and then spends the named pool, disabling only when BOTH are empty (Telekinetic Movement / Psi-Powered Leap / Bulwark of Force are each free once per rest, then cost a die — found in QA, where a spent free use left "0 / 1 remaining" and a dead button while four legal uses remained); and **`restoresResourceKey`** / **`restoresKey`** = SPEND-TO-RESTORE, one atomic patch spending the row's charge and handing a use back to the named pool (the bonus-action "regain one expended Psionic Energy die", which the backend had been resetting on every rest while the UI gave nobody a way to spend it). The regain is **the one sanctioned exception to "a player never restores a resource"** — the restore IS the feature firing, gated behind its own once-per-rest charge — and carries **`hidden(ctx)`** (both surfaces) so it disappears while the pool is full rather than offering a button that does nothing. A two-cost control also LABELS each count ("Free use 1 / 1 · Psionic Energy Dice 4 / 4"): a bare "1 / 1" beside a feature that also spends a shared pool reads as "you can do this once". Authored once for both editions, since the 2024 revision renames nothing and moves no level. Its L18 Telekinetic Master became **two cards, one of them a COMBO** — a `Cast Telekinesis` action card, plus a `Telekinetic Master` entry in the **`action+bonus`** bucket — after QA found the original single card badged 'bonus action' while its headline half, casting the spell, is an action. **Why a combo and not a lone bonus action:** RAW telekinesis is exerted *as your action each round*, so the bonus attack only ever happens on a turn whose action IS telekinesis (on the first turn the action casts it — you are concentrating from that moment, so the attack is already live; after that the action re-exerts it). Filed under `bonus` the card sat alone in the Bonus Actions tab with nothing naming the action that enables it, which is not something a player can act on. It carries **`telekineticAttack: true`**, which builds two halves. The `Action` half is a normal `subAttacks` row naming Telekinesis; it is not an attack, so it takes `detail` instead of to-hit/damage (the Tavern Brawler Grapple shape). The `Bonus` half is the new bit: **`bonusEntries` — REFERENCES to the real weapon entries in the Actions bucket, rendered by the tab as full nested attack cards**, not summary rows. QA drove this: the bonus attack is an *ordinary weapon attack*, so taking it needs the range band, Psionic Strike's Use control, the Sharpshooter toggle, spacing and ammunition — none of which a `{name, toHit, damage}` row can show. They are references and deliberately not copies, so anything attached to a weapon later in the pipeline (Psionic Strike/Telekinetic Thrust from earlier subclass features, the ATTACK_RIDERS applied at the very end) is picked up free rather than frozen mid-build; the tab **prefixes the key at RENDER time** (`{comboKey}-bonus-{weaponKey}`) so the nested copy's testids can't collide with the same weapon's own Actions-tab card while the entry stays live until drawn. Unlike Unwavering Mark the rows are `scope`-free (RAW says "a weapon attack", so a bow and an unarmed strike both qualify) and fold in NO damage bonus, so each mirrors the normal attack exactly. Its concentration gate is TEXT on the card rather than a filter, since the app models no concentration state (the line Rage and durations sit behind). The at-will *telekinesis* itself now lives in the Spells tab's Subclass source — see `subclassSpells.js` below. **Extra damage is FOLDED INTO the card's one damage number** — `applyDamageAdditions(view, additions)` → `{damage, damageBreakdown}`, feeding the same clickable `AttackNumber` the to-hit uses, so an attack shows ONE damage value with the math a click behind it. It used to print the plain swing plus a second "on a hit" total beneath it; QA reported that as "doubled damage", since both lines opened with the same string and neither said which to use. An addition of the weapon's own type (or an untyped one, which RAW means weapon damage) joins the DICE GROUP — `1d8 + 1d6 + 4 Piercing`, one roll — rather than trailing after the type word; a different type still keeps its own term, because piercing and fire are rolled and resisted separately. Dice are never summed into one die. **What may be folded in is narrower than what may be mentioned**: only damage that is *currently happening*, which today means an `active_effects` toggle the player has switched ON — `ATTACK_RIDERS.damage(row, ctx)`, i.e. **Giant's Might** alone (weapon-typed via `weaponDamageType`, scaling 1d6→1d10). Damage that costs a spend decided AFTER the hit stays out and lives in its feature's own block instead, where the number is still on the card beside the Use control that pays for it: the **Fire Rune**'s 2d6 and — as of this QA pass — **Psionic Strike**'s die + INT force, which had been inflating every swing a Psi Warrior might never pay for. So `ATTACHED_FEATURES.damage` currently has no authors; declare one only for damage that needs no in-the-moment choice. This is the standing Sneak Attack/Divine Smite rule narrowed, not lifted — what changed is that an active effect is a conditional the app actually knows about. Every folded-in term names its source in the breakdown (a number that grew because an effect is running would otherwise read as a bug in the weapon damage) plus its optional **`when`** — Giant's Might says "once per turn", since the printed number is per-attack. The data layer hands the card a LIST (`entry.damageAdditions`), not a finished string, and the card folds it into the damage it is CURRENTLY showing — a baked-in total would go stale the moment the Great Weapon Master / Sharpshooter toggle rewrites the damage. Ordering matters: the collection pass runs LAST, after ATTACK_RIDERS, since a rider is attached after the features are. **`attachedFeatures` is the generic "limited-use feature that rides on THIS attack" slot** — an array on a weapon-attack entry of `{key, name, note, resourceKey}`, rendered by `AttachedFeatureBlock` inside that attack's card with its own Use control (`ae-attached-{slug}-{entryKey}`). The control sits INSIDE the block, never at the card's top-right, so the remaining count reads as the feature's and not the weapon's ("5 / 5 remaining" beside a longsword claims the longsword has five uses). **It is now a data table, `ATTACHED_FEATURES`** — `{name, scope:'melee'|'ranged'|'all', resourceKey?, note(row, ctx)}`, opted into by a subclass entry's **`attachedAs: '<name>'`** — extracted at the SECOND consumer, the same consolidation trigger that produced ATTACK_RIDERS/ENTRY_RIDERS. When no attack matches the scope, the entry falls through to its standalone card, so the feature never vanishes (the Arcane Shot / Weapon Bond fallback shape). Authored: **Unleash Incarnation** (Echo Knight L3+, melee) — found in QA sitting in the No Action tab, where a feature you reach for mid-Attack-action is unfindable; it attaches to every melee attack row (`e.melee`, so an unarmed strike qualifies) with the weapon named in the note, so a bow-only Echo Knight never loses it. **Psionic Strike** (Psi Warrior L3+) takes `scope:'all'` — RAW keys on "a weapon attack", not a melee one, so a bow qualifies; an unarmed strike does too, which is why a bare-handed Psi Warrior keeps it on that card instead of falling back. **Fire Rune** (Rune Knight) is the counter-example for `damage`, and the line it draws: it deliberately declares NONE, so its 2d6 fire never joins the card's damage. The fire is not a property of the weapon — it lands only on the swing where you spend a Channel Rune use to summon the shackles, a choice made after the hit and never recorded — so it stays prose in the note beside the Use control that invokes it. **Psionic Strike is now the same call** (its `damage` spec was removed in QA): spending a Psionic Energy die is the same after-the-hit choice, and folding it in claimed force damage on every swing. Declare `damage` only for extra damage that needs no in-the-moment choice. **Telekinetic Thrust** (Psi Warrior L7+) carries no `resourceKey` at all: it costs nothing and fires on a Psionic Strike hit, so it rides the same cards rather than taking a card of its own. Arcane Shot deliberately keeps its own richer `arcaneShot` block (an options menu + a save DC, neither expressible here); fold that into `attachedFeatures` if a second rich one appears rather than writing another block. A subclass entry may now carry the same **`compute({characterData, level, scores})`** the racial map has — Ferocious Charger's Strength save DC scales with level and STR, so a fixed string would show the wrong number to everyone but one character. A `compute` may return a **`saveDc: {label, breakdown}`** instead of writing the arithmetic into `detail`: the tab renders it as a clickable `BreakdownValue` (`ae-save-dc-{key}` → `ae-save-dc-breakdown-{key}`), the same treatment the Arcane Shot DC gets. Ferocious Charger used to inline "DC 15 = 8 + PB +5 + STR +2" mid-sentence, which made the rules text unreadable for math nobody needs until they question the number. **Cavalier also demonstrates riding on a UNIVERSAL entry**: Hold the Line (L10) changes what the shared Opportunity Attack reaction does rather than adding an action, so it attaches as a `rider` on `universal:Opportunity Attack` (the Arcane Charge-on-Action-Surge shape). That block must run AFTER the universal menu is pushed, since that's what creates the entry it attaches to. **Vigilant Defender (L18) is the counter-example, and the line between the two:** it grants a whole SEPARATE reaction (once on every other creature's turn, not spending your normal one), so it is its OWN entry carrying **`extraReaction: true`**, which the tab renders in a bordered **"Extra reactions — on other creatures' turns"** section (`ae-extra-reactions`) below the normal groups. As a rider it was invisible — the Reactions tab showed a lone Opportunity Attack and nothing told the player they had a second reaction economy. Rule of thumb: a rider MODIFIES an action you already had; a feature that ADDS one gets an entry. **Arcane Shot demonstrates the "attach to the attack it rides on" shape**: rather than a free-floating No Action entry, a qualifying shortbow/longbow attack entry carries an `arcaneShot` payload (cost, save DC, each known option with its description) plus the shared `arcane_shot_used` resourceKey, and the tab renders it as a block inside that weapon's card with the Use control in it — so the option is read off the same card as the attack. It falls back to a standalone entry only when no bow is equipped (RAW excludes crossbows), so the feature never vanishes. **Unwavering Mark demonstrates SPLITTING one feature across two surfaces by cost** — it is two mechanics at two prices, and one card could only lie about one of them. *Marking* is free, unlimited and fires on a melee weapon hit, so it attaches as a **rider on every melee attack card** (`e.melee`, a flag now on each weapon entry — an **unarmed strike counts**, hence its null-weapon branch). The *follow-up attack* is the limited half, so it keeps a bonus-action card, renamed via a `displayName` on the subclass map entry to **"Marked Target"** — the trigger a player scans for mid-combat, not the feature name — and carries computed `subAttacks`, one per melee attack, with **half the Fighter level folded INTO the damage string** (it is one attack rolled once, so a trailing "+3" would read as a second damage source) plus a "with advantage" note. The card is **omitted entirely** when there is no melee attack row (bow-only), rather than shown with an instruction; bare-handed still gets it, since the unarmed strike is a melee row. **The ability term in both the to-hit and damage breakdowns names the RULE that chose it** (`weaponAbility` now returns a `reason`): 'DEX (ranged weapon)', 'STR (Finesse — STR ties DEX)', 'CHA (Hex Warrior)', or a plain 'STR' when no rule applies. Found in QA — a Finesse scimitar at STR 14 / DEX 15 showed a bare "+2 STR", the correct number but indistinguishable from the app hardcoding Strength. `addFlatDamage` gained a flat-damage branch for this: a base unarmed strike is "4 bludgeoning" with no die, and the old die-first regex turned +3 into "4 + 3 bludgeoning" instead of "7". **Riders are now COLLAPSIBLE** (via the shared `shared/FeatureNote.jsx`, which also backs the Items-tab equipment notes): name only, rules text on click (`ae-rider-{slug}` → `ae-rider-{slug}-text`) — a rider is a paragraph you need only once its trigger fires, and expanded ones bury the attack numbers the card exists for. Arcane Charge/Hold the Line/Vigilant Defender inherit this. **Riders that hang off WEAPON ATTACK cards are a data table, `ATTACK_RIDERS`** — `{source, scope:'melee'|'ranged'|'all', applies(ctx), text}` — consolidated at the point a third hand-written attach block would have appeared. Authored: **Unwavering Mark** (Cavalier L3+), the **Mounted Combatant** feat (melee only, gated on `hasFeat`) — both land on an unarmed strike too, since `scope:'melee'` follows the entry's `melee` flag — and the **Sharpshooter** feat (`scope:'ranged'`, both editions — Bypass Cover is the one clause the two versions share), the first rider that carries only PART of a feat — and now only its **cover** clause, because every other clause became a NUMBER on the same card: the long-range clause moved onto the range band (below), the −5/+10 is the power-attack toggle, and the within-5-ft rule was never Sharpshooter's (it is Crossbow Expert's, already handled by `spacingNote`). Cover stays prose because there is nothing to attach it to — the app has no distance-to-target model. One edition-neutral row serves both editions. A rider's **`text` may be a FUNCTION of the rider context**, not only a string — **Giant's Might** (Rune Knight, `scope:'all'`, so an unarmed strike qualifies) states a die that scales 1d6 → 1d8 (Great Stature) → 1d10 (Runic Juggernaut), which a static string could not. Its `applies` also gates on the effect actually RUNNING, not merely being earned: a rider is rules text read mid-swing, and a paragraph for a feature that is switched off is clutter on the surface a player scans fastest. Switching it on is what makes both the block and its damage term appear. It is a RIDER rather than part of the flat damage string for exactly the Sneak Attack/Divine Smite reason — it is once-per-turn — but unlike those the app now KNOWS whether it applies. A thrown handaxe keeps its melee card and so does NOT get it, the same line `isPureRangedWeapon` draws for the toggle. `text` is authored in the table because FEAT rules text lives only in the backend compendium and this module is pure. **Riders that attach to ONE NAMED entry are the sibling table, `ENTRY_RIDERS`** — `{source, entryKey, applies(ctx), text}`, matched by key across every bucket rather than by weapon scope, and applied LAST so a rider can target the universal menu (pushed last) as easily as a class feature. Consolidated at the same third-instance trigger, migrating the two hand-written blocks unchanged. Authored: **Arcane Charge** → Action Surge, **Hold the Line** → Opportunity Attack, the **Sentinel** feat → Opportunity Attack (its speed-0 + provoke-through-Disengage clauses; they were authored in the backend only as a display-only `note`, so the Reactions tab showed the unmodified stock rule text — found in QA), and the **Polearm Master** feat → Opportunity Attack (creatures provoke on ENTERING your reach — the same display-only-note gap). Polearm Master is the first **equipment-gated** rider, and the reason `applies` is handed the `inventory`: the clause is true only while a qualifying polearm is equipped, so it hides otherwise (matching how the feat's bonus-attack half is already gated) rather than telling a longsword user something false. It needs its OWN weapon predicate (`isPolearmReachWeapon(edition)`, not the bonus attack's `isPolearm`) because RAW the feat's two halves take **different** lists — 5e's reach clause drops the spear and adds the pike, and 2024 restates it as quarterstaff/spear/any Heavy+Reach weapon (so that edition tests properties, not names, and picks up homebrew polearms free). The compendium's seeded feat text flattens both halves to one list; the gate follows the rulebook. Rider `text` stays edition-neutral and names no weapons — the gate knows the list and the card only exists when you hold one. Sentinel is also the rider/entry line drawn twice in one feat: those two clauses MODIFY the opportunity attack, while **Sentinel Strike** fires on a different trigger (a creature within 5 ft attacking someone else) and is not an opportunity attack at all, so it stays its own Feat reaction entry — the same call Vigilant Defender got, minus `extraReaction` since it spends your normal reaction. Further OA riders (War Caster's cast-a-spell-as-an-OA, Mage Slayer) are now one-row data entries. **`RACIAL_ACTIONS` entries can be COMPUTED** — an entry may carry `compute({characterData, level, scores})` that derives its own detail from the character, plus a `resourceKey` into `RACIAL_REST_RESOURCES` (the tab merges racial resources alongside the class config's, so the card gets a Use button writing the same `<key>_used` as the Stats tab). Wired: **Dragonborn Breath Weapon** — damage (2d6→3d6@6→4d6@11→5d6@16), save DC (8+CON+PB), save ability and area all resolved from level + the stored `draconic_ancestry` via `race/breathWeaponData.js`, instead of the old static string that showed a level-1 card at level 16. **Grappler (feat, both editions) gets a `Grapple Attack` CARD, not a rider** (user's call: leave as little as possible to text) — `feat:grappler-attack`, one `subAttacks` row per weapon card carrying that attack's real to-hit + folded damage "with advantage"; it introduced two generic entry fields, **`origin`** (a badge naming where a card comes from, `ae-origin-{key}`) and **`requirement`** (an amber condition line the app can't check, `ae-requirement-{key}` — here "Only while you're grappling a creature", since there is no grappled-target state). The 2014 restrain is the feat's own `Pin: Restrain` action effect (it costs the whole action, so it can't share the card); 2024's Punch and Grab + Fast Wrestler are an ENTRY_RIDERS row on `universal:Grapple` whose text names the character's own size — entry-rider `text` may now be a function of the context (which carries `size`). **Grapple and Shove are cards in the universal menu**, from the new `combat/specialAttacksData.js`. They are deliberately NOT filed as actions: in neither edition is either one an action of its own (2014 calls them special melee attacks made in place of an attack; 2024 makes them two of the three Unarmed Strike options), so each carries the cost badge **"replaces one attack"** — badging them 'action' would teach that you must choose between grappling and swinging, when Extra Attack lets you do both. Each card computes its own **size line** (`ae-size-{key}`) from `creatureSize` WITH the active-effect context — "You are Medium — you can grapple a creature up to Large." — which is how a running **Giant's Might** moves the ceiling to Huge without the module knowing the rune exists, and is also **the only place the app prints the character's size**, the gap left open when the ActiveEffectsBanner was removed. The character's own size is stated FIRST because the limit follows from it. A grapple's free-hand requirement is a WARNING on the card (the Tavern Brawler shape — never a reason to hide it), and it is edition-aware: a 2014 shove needs no free hand while a 2024 one does. Entries carry a generic **`mechanicsSlug`/`mechanicsLabel`** that the tab renders as a "Learn more" link (`mechanics-learn-more-{key}`), reusable by any future entry whose rules outgrow a card. **The universal group's `ItemRow` previously received NO props at all** — found here, since its new link rendered `/campaigns/undefined/…`; it is now passed `campaignId`. **Spells now get a card EACH, filed under the action they cost** (they used to collapse to one "Cast a Spell" pointer per bucket to avoid clutter — collapsible source groups removed that constraint, and the pointer answered "can I cast something?" when the mid-combat question is "which of my spells is a bonus action?"). The new **`castableSpells({characterData, charClass, subclass, level, edition, race, subrace})`** resolves all FOUR sources the Spells tab shows — class lists, racial (cantrips + `getRacialSpellResources`), subclass (`subclass_cantrips` + `getSubclassGrantedSpells`) and feats (`getFeatGrantedSpells`) — as `[{name, source, resourceKey}]`, deduped with first-source-wins so a spell held twice is one card keeping its class attribution. **This closed the standing racial gap**: `characterSpellNames` read only cantrips/prepared/known, so a Tiefling's Hellish Rebuke — a *reaction* — was absent and the Reactions tab looked empty for a character who plainly had one. The tab's FETCH GATE uses the same resolver, or a character whose only spells come from a race/subclass/feat downloads no catalog and every spell is silently unclassifiable. A card carries level · school · Concentration · Ritual · "From {source}" (omitted for the class list, where it is noise), sorted cantrips-first then by level; a source that METERS the spell passes its `resourceKey` through (a racial once-per-rest, a feat's `free_cast`), so the card gets the standard Use control writing the SAME key the Spells-tab row writes — the WeaponAmmoControl arrangement. Ritual-book spells (Ritual Caster) are excluded: castable only as rituals, which is never a combat action, as are compendium-unknown spells and any casting time longer than a turn. Remaining: author `CLASS_FEATURE_ACTIONS_*` for the other 12 classes (+ more subclass features), establish the "rider" convention for no-action abilities (Sneak Attack, Divine Smite, Rage), and wire chosen options into the action economy (Battle Master maneuvers — now stored in `character_data.maneuvers`, Sorcerer metamagic, Warlock invocations — not yet stored).
...
- Feat selection UI — built for 5e: feats live in the encyclopedia (Feats tab, edition-aware), the Variant Human creation flow picks one feat + skill, the **LevelUpWizard ASI levels** offer ASI and/or a feat per the GM's `campaign.asi_feat_mode` (asi_only / asi_or_feat / asi_and_feat), and the **CharacterDetail Features → Feats sub-tab** (`FeatsSubTab`) lists owned feats with descriptions + GM add/remove. All stored in `character_data.feats` as `{id, name}`. **5.5e (2024) ASI/feat flow: TODO** — the LevelUpWizard feat step is edition-aware and functions for 2024 campaigns, but 2024-specific feat handling is unvalidated: 2024 feat categories (Origin/General/Fighting Style/Epic Boon) and **half-feats that grant a +1 ability score alongside their benefit** (the wizard doesn't yet apply a feat's bundled ASI). **Feat effects model — built (5e vertical slice).** Feats carry a structured `effects` JSON array (backend `feats.effects`); the frontend `featEffects.js` resolves them into real mechanics — `stat_mod` (Alert → +5 initiative on the Stats tab), `ability_choice`/`ability_score` (half-feats like Tavern Brawler → ability chooser at acquisition, folded into the score), `attack_mod` (Tavern Brawler → 1d4 unarmed in Action Economy), `action` (Tavern Brawler grapple → TWO Action+Bonus combos, one per RAW opener — Unarmed Strike + Grapple and Improvised Weapon + Grapple; the free-hand requirement a grapple needs is a WARNING on the card, never a reason to hide it — hiding it made the feat invisible to any Fighter holding a weapon and a shield), `proficiency` fixed weapon grant (Tavern Brawler → "Improvised weapons" in the Items-tab banner + per-weapon proficiency check + an **Improvised Weapon attack row in the Action Economy Actions bucket**, built from the feat so a character who owns no such item can still swing a chair; an "Improvised Weapon" is also a seeded system weapon you can equip, whose own row then replaces the generic one), `note` (display-only). Effects are snapshotted onto `character_data.feats[i]` at acquisition and shown as chips in the Feats sub-tab. `report_feat_effects.py` lists per-edition mechanized-vs-prose-only coverage. **Status: 5e 26/41 feats mechanized** (see `report_feat_effects.py`). Consumers live: `stat_mod` initiative (CharacterDetail derived row) + passive_perception / passive_investigation / passive_insight (the Abilities & Skills **Passive Scores** row, via `skills/passiveSkills.js` — siblings `skills/skillMath.js` (shared arithmetic) + `skills/BreakdownValue.jsx` (the clickable number + its expandable panel) put a click-to-see-the-math breakdown behind EVERY derived number: skills, saving throws, passive scores and initiative — a passive is 10 + the skill modifier, so proficiency counts and expertise doubles it) + speed (CharacterDetail annotation `speed-feat-note`); `ability_score`/`ability_choice` (LevelUpWizard half-feat chooser **and** Variant Human *creation* chooser `human-feat-ability-{stat}`); `action` (Action Economy); `attack_mod` unarmed die; `resource` pools (Lucky, Martial Adept — FeatsSubTab tracker via RestResourceControl + backend `_compute_rest_patch` reset + getRestSummary); `proficiency` **fixed** grants (armor → Items-tab banners via gatherProficiencies) + **saving_throw** (Resilient → saves display) + **count-choice pickers** (Skilled 3 skills/tools, Linguist 3 languages, Weapon Master 4 weapons — `featProficiencyData.js` drives count-limited pickers in the LevelUpWizard feat step + Variant Human creation; picks merge into skill_proficiencies / feat_tool_proficiencies / feat_languages / feat_weapon_proficiencies and surface in banners + the "From Feats" languages group); `note` (display-only). 2024 feats are authored too (`FEAT_EFFECTS_2024`, 50/73) and consumed by the same edition-agnostic resolvers. **PB-scaled** values are supported: a `stat_mod` `amount` or `resource` `total` of `'pb'` resolves against the proficiency bonus the consumer passes (`getFeatStatMods(feats, stat, {pb})`, `getFeatResources(feats, {pb})`) — drives 2024 Alert's initiative-proficiency + Lucky's PB luck points. `expertise` (Skill Expert) is wired too — a count-choice grant whose pool is the character's proficient skills (incl. a skill picked from the same feat), routed to `expertise_skills`. **Conditional AC** is wired via `ac_mod` effects evaluated inside `computeArmorClass` (which has the equipment context): Defense (+1 while wearing armor), Dual Wielder (+1 with two equipped melee weapons), Medium Armor Master (raises the medium-armor DEX cap to 3). **`save_mod`** is its sibling for saving throws (`getFeatSaveMods` → `conditionalSaveBonuses(ability, {feats, inventory, pb})` in `skills/saveFeatures.js`): a bonus that applies only SOMETIMES, carrying an equipment `condition` the resolver evaluates (`shield` → `equippedShield`), an `amount` that may be the `'pb'` or `'shield_ac'` sentinel, and a `situation` string NOTHING can evaluate. Authored: **2014 Shield Master** (+shield AC to DEX saves vs. effects that target only you; the 2024 feat deliberately has no counterpart — that edition dropped the clause). **It IS summed into the printed save** (the user's call, made against the initial build, which had deliberately kept it out). That makes the total a BEST CASE — too high against a fireball, which targets more than you — so the restriction and the feat name are carried by every surface that shows the number, and the resolver builds both strings so no surface can show one without them: `part` (a breakdown term labelled "Shield Master (only against effects that target only you)") and `text` (the inclusion-worded `saves-conditional-note-{key}` line under the grid, "DEX saving throws include +2 from Shield Master — but only …"). The raised total is COLOURED, the same marker the active-effect bonuses use — in this grid a coloured total means "includes something named below", whatever the source, and a +3 on a DEX 12 character would otherwise read as a bug in the modifier. Never mark it with a second number beside the total. Authoring rule: **give every `save_mod` a `situation`**, since one without it would silently claim an unconditional bonus. It had shipped as a display-only `note` doing nothing (found in QA). **`save_advantage` is its sibling for ADVANTAGE** (`getFeatSaveAdvantages` → merged into `getSaveFeatures`, so feat rows appear in the **Features Affecting Saves** panel labelled `Feat`, appended after the class features because a feat has no gained-at level). `abilities` may legitimately be EMPTY — RAW often names none ("saving throws to avoid or resist traps" is any of the six) and guessing one narrows the feat. Its description is built from the effect by `saveAdvantageText`, NOT from the compendium: the `character_data.feats` snapshot carries no description at all. **The `situation` field is load-bearing**: `advantageAbilities` is `situation ? [] : abilities`, so a situation-scoped advantage tags NO save row — tagging CON for War Caster would tell the player to roll every Constitution save twice. Authored: **War Caster** (both editions, CON to maintain concentration), **Mage Slayer 5e** (vs. spells from creatures within 5 ft; the 2024 feat dropped the clause), **Dungeon Delver** (vs. traps). **`stat_mod` speed** folds into the shared **CombatBlock** Total Speed for data-driven sheets (Fighter/Wizard, `total-speed`/`total-speed-feat-note`); the hand-written sheets — which can't yet — show a central CharacterDetail annotation (`speed-feat-note`, suppressed when `getClassConfig` finds a config), so it drops away progressively as classes migrate via `/class-config`. The wizard's feat **prereq gating now covers the armor bucket** too — it derives the character's armor proficiencies from the class table + race grants + feat-granted armor (so the ladder works: Lightly → Moderately → Heavily Armored), so a caster sees armor feats locked. The feat-effects consumer set is nearly complete (clause-level: 5e 23/41 feats complete + 18 clauses pending, 5.5e 29/73 + 44 — `report_feat_effects.py` lists every pending clause). **`spell_grant` (Magic Initiate, both editions) — BUILT (vertical):** the effect + resolvers (`getSpellGrantSpecs` / `getFeatGrantedSpells` → `{cantrips, leveled, freeCasts}` + `featFreeCastUsedKey`, snapshotted to `choices.spell_grant`); the `FeatSpellGrantPicker` wired into the LevelUpWizard feat step + Variant Human creation (picks a list + ability + cantrips + a 1st-level spell, blocks Next via `spellGrantComplete`); the CharacterDetail **Spells tab Class/Racial/Feats source sub-tabs** (`spell-source-{key}`, only non-empty shown) with the Feats section (`FeatSpellsSection`) showing the granted spells (per-level `SpellLevelTabs`) + a **1/long-rest free-cast tracker** (`feat_freecast_<slug>_used`, reset on a long rest by `_compute_rest_patch` + mirrored in `getRestSummary`). Also authored: **Spell Sniper** (5e class cantrip / 2024 group cantrip), **Telekinetic** (fixed Mage Hand cantrip + bonus-action shove `action`), **Telepathic** (fixed Detect Thoughts), **Fey Touched** (fixed Misty Step + a chosen Divination/Enchantment L1), **Shadow Touched** (fixed Invisibility + a chosen Illusion/Necromancy L1) — via the extended spell-grant model: a `spell_grant` may carry always-granted `fixed:[{name,level}]` spells (shown read-only); a leveled slot may carry a `school:[...]` filter (chosen from any spell of that school, no class list); `ability:'none'` skips the ability picker when a 2024 half-feat's ASI already sets it; and `free_cast:'long_rest'` means **every leveled granted spell (fixed level≥1 + chosen leveled) is a 1/long-rest free cast** (so Fey/Shadow give two). `resolveSpellGrantValue(spec, value)` snapshots the final shape — picks + `fixed` + a `free_casts:[names]` list — into `choices.spell_grant` (pure-fixed/school feats need no source pick so they auto-complete). `_compute_rest_patch` + `getFeatGrantedSpells` read the `free_casts` list (tolerating an older singular `free_cast`). **Ritual Caster** is also done via a `ritual: True` leveled slot: the picker filters that slot to ritual-tagged spells of the chosen class, and `resolveSpellGrantValue` converts the picks into a **growable, editable `ritual_book:[names]`** (cast as rituals only — no free cast); `getFeatGrantedSpells` returns `ritualBooks:[{featIndex, source, spells}]`, which the Spells-tab Feats section renders as an editable `SpellList` (add/remove persists onto the feat instance via an `{feats}` patch). **All spell-granting feats are now authored** (Magic Initiate, Spell Sniper, Telekinetic, Telepathic, Fey/Shadow Touched, Ritual Caster). **`maneuver_grant` (Martial Adept) — BUILT (5e):** the effect `{count, die}` + resolvers (`getManeuverGrantSpec`/`maneuverGrantComplete`/`getFeatManeuvers`/`martialAdeptDieCount`/`martialAdeptManeuverCount`); the `FeatManeuverPicker` wired into the LevelUpWizard feat step + Variant Human creation (pick N maneuvers, excluding any already known, blocks Next), snapshotted to `choices.maneuvers`. A **Battle Master** folds the feat's +1 die (at their die size) and +N maneuvers into the shared Combat Superiority pool (`BattleMasterPanel`) with the picks merged into `character_data.maneuvers`; a **non-Battle-Master** gets a standalone d6 (the existing `martial_adept_superiority` resource, reset on a short rest by `_compute_rest_patch`) + the chosen maneuvers shown in the `FeatsSubTab` maneuver panel. Author more via `/feat-effects`. Authoring procedure: the `/feat-effects` skill.
- Level-up pool choices — **generic mechanism built + proven on four pools**: **Sorcerer Metamagic**, **Warlock Eldritch Invocations**, **Arcane Archer Arcane Shot**, and **Rune Knight Rune Carving** (`levelChoicesData.js` + the LevelUpWizard `level-choices` step). Each new pool drove one small generic addition and no new component code — option-level gating via `minLevel` (`availablePoolOptions(…, level)`) for invocations; a **`subclass` field** for Arcane Shot, so a pool can belong to a SUBCLASS rather than the whole class (`getLevelChoices(…, subclass)` is passed the wizard's `effectiveSubclass`, so a subclass chosen in the same run still gets its pool) plus `getEarnedLevelChoices` + optional `improvementAt` / `derived(level, scores)` for the sheet display. **Rune Carving added NOTHING** — the fourth pool was pure data (`runesData.js` + one `LEVEL_CHOICES` entry), combining `subclass` scoping, option `minLevel` (Hill 7 / Storm 15) and `derived`, which is the check that the mechanism is done rather than accreting per-pool code. Further pools are pure data entries: Expertise (Rogue 1/6, Bard 3/10), Fighting Style for non-Fighters (Ranger/Paladin), Pact Boon, etc. (Battle Master Maneuvers have their own dedicated step.) The chosen options display read-only on a data-driven sheet via the shared **`KnownOptionsBlock`** (owed-slot picker, GM-Edit remove, level-gated improved option text — past `improvementAt` an option's `improvedDescription` REPLACES its description, resolved for every surface by `poolOptionDescription`, so nobody appends an "…increases to 4d6" rider that reads as extra damage — derived line) — deliberately pool-agnostic rather than a second `BattleMasterPanel`. **Replace-on-level-up is built** — the `level-choices` step (Metamagic, Invocations), the Battle Master `maneuvers` step, and the known-caster `spells` step all let the player swap one held option/maneuver/spell when leveling (via a `ReplaceOneSelect` for the pools/maneuvers, or just remove+add in the spells step). **Remaining gap**: full *prerequisite* gating beyond level (Pact Boon / specific cantrip / another-invocation deps — only `minLevel` is enforced; other prereqs are noted in the option description). The hand-written sheet free-text inputs remain the fallback.
- **Summoned companions — mechanism built, 1 entry authored** (`companions/companionData.js` + the shared `CompanionPanel`). A companion is an entity a character conjures that has its own defensive numbers but no sheet of its own; the table is class-agnostic (`charClass`/`subclass`/`edition`/`minLevel`) and each entry supplies a `count(level)`, `stats(level)` and `traits(level)`. A stat carries either a plain string or a `buildBreakdown`, so a derived number is a clickable `BreakdownValue` like every other one on the sheet (`companion-{key}-stat-{stat}` → `…-breakdown`); traits collapse to their name via the shared `FeatureNote`. Rendered in the ClassSheet's **subclass features** area — data-driven, NOT registered in `config.subclassPanels`, so it needs no config entry and any config-driven class gets it free (a hand-written sheet does not; that's the same progressive gap as `speed-feat-note`). Authored: the **Echo Knight's echo** (AC 14 + PB via the exported `echoArmorClass`, which the Action Economy Manifest Echo card also reads so the two can't drift; 1 HP, conditions immunity, 1-minute duration, 30-ft leash, and two echoes at L18 for Legion of One). **Deliberately stateless** — no current HP, no "is it out right now", no position: the app displays what a character can do rather than simulating a battlefield (the Encounters-V1 line), and a 1-HP summon has nothing to track. A companion with real hit points (a familiar, a Steel Defender, a Drakewarden's drake, a mount) wants the Bestiary tie-in, not a bigger version of this table — see the mounts bullet below.
- **Mounts & vehicles — NOT MODELLED AT ALL.** Nothing tracks whether a character is mounted, what they're riding, or the mount's own AC/HP/speed/saves. Every mounted rule in the app is therefore prose: none of it can be computed, level-gated by the mount, or rolled. **When mounts/vehicles are built, revisit the mounted features already shipped as prose and decide which become real mechanics** — Cavalier **Born to the Saddle** (advantage on saves to avoid falling off; mount/dismount costs 5 ft instead of half your speed — the speed half could feed the movement model), Cavalier **Unwavering Mark**/**Ferocious Charger**/**Hold the Line** (all written assuming a mounted charge), the **Mounted Combatant** feat (advantage vs. smaller unmounted creatures — needs a target model; redirect-attack-to-self — needs a mount entity; **the mount's** Dexterity-save evasion — belongs on the MOUNT's sheet, which is why it is deliberately NOT in the Features Affecting Saves panel, whose scope is the character's own saves), and the **Mounted Combatant 2024** variant. Also unbuilt: mount/vehicle statblocks, the Bestiary tie-in (a mount is a creature), and speed/carrying rules. A mount is the same shape as a summoned companion (above) but with real hit points, so whatever models it should subsume `companionData`'s one stateless entry rather than sit beside it. Until then the honest surface for all of it is a prose rider or a feat card.
- Features gated on the EQUIPMENT you hold — **mechanism built, 6 entries authored** (`inventory/equipmentFeatures.js` + `ITEM_SCOPES`). An Items-tab row shows the features that the item is a prerequisite for, collapsed to the feature name. Authored: **Warding Maneuver** (Cavalier L7 — melee weapon or shield), **Savage Attacks**, **Great Weapon Master**'s crit/kill bonus attack, **Eldritch Strike**, 2024 **Remarkable Athlete**'s move, **Heavy Armor Master** (the `heavyArmor` scope — deliberately narrower than `bodyArmor`, since the feat does nothing in light/medium and showing it there would state a benefit the wearer lacks; its text is built from the same `damage_reduction` feat effect the Defenses panel reads, so the number can't drift between the two surfaces). Adding one is a data entry. Un-authored candidates, all the same shape: **Protection** and **Interception** fighting styles (shield / shield-or-weapon reactions), **Shield Master** (shield → bonus shove; its DEX-save bonus is already mechanized as a `save_mod` feat effect, so only the shove clause is note-worthy here), **Defensive Duelist** (finesse weapon → +PB AC reaction), **Polearm Master** (polearm → bonus attack + reach OA), **Sentinel** (any weapon → OA riders), **Dual Wielder** (two melee weapons — note its +1 AC is already folded into `computeArmorClass`, so only the draw/stow clause is note-worthy).
- **Rune Knight (Fighter, 5e only — the 2024 PHB ships no Rune Knight) — IN PROGRESS.** **Feature-text correction (QA):** the stored `Giant's Might` blurb claimed "you can grapple Large creatures" and `Runic Juggernaut`'s claimed "grapple creatures of Huge size or smaller". Neither clause is in the feature. Grappling is governed by SIZE — no more than one size larger than you — so a Medium character **already** reaches Large, and what Giant's Might really does is make you Large, moving the ceiling to Huge (Huge → Gargantuan at 18). The blurbs therefore granted nothing while teaching a baseline one size too low, **and a player reading them concluded exactly that** — they reported the new Grapple card as wrong because the feature text said otherwise. Corrected in `subclassData/fighter.js`: the invented clause is gone, the advantage is narrowed to "Strength checks and Strength **saving throws**" (matching what `grants` returns — the old "saving throws" contradicted the tags the sheet draws from it), the once-per-turn/unarmed-strike scope of the 1d6 is stated, and Runic Juggernaut's real second clause (+5 ft reach while Huge, already implemented as `reachBonus`) replaces the grapple line. Guarded by tests in `activeEffects.test.js`, since these strings are read back out of SUBCLASS_DATA by the Defenses and save-features panels. Built: **Giant's Might** (bonus-action card + PB/long-rest pool + the active-effect toggle + the scaling damage rider + the grown size) and **Runic Shield** (reaction card + PB/long-rest pool), both reset by the backend long rest and mirrored in `getRestSummary`; and **Rune Carving** — a `levelChoicesData` pool of the 6 runes (`runesData.js`), 2→3→4→5 known at levels 3/7/10/15, Hill gated at L7 and Storm at L15 via option `minLevel`, swap-one-per-Fighter-level, stored in `character_data.runes`, with the save DC (8 + PB + CON) on the choice's `derived` line and the known runes shown by `KnownOptionsBlock`. It cost NO new component code. *(QA found the pick never happened at all — a Rune Knight leveled to 3 and was simply never asked. A character who leveled before this existed knows no runes and is offered the owed slots on the sheet.)* Also built: **rune → item assignment** (`character_data.rune_items`, the control on the Items-tab weapon/armor/shield rows via `RuneCarvingControl`; see the schema note — knowing a rune grants nothing, `activeRunes()` requires carved AND equipped), **Channel Rune** (six `hidden(ctx)` rest rows + six Action Economy cards generated from `RUNE_OPTIONS`, one per rune, each with its own counter, 1/short-or-long rest → 2 from **Master of Runes** at L15; the Fire Rune's rides on a weapon attack so it ATTACHES to the attack cards via `ATTACHED_FEATURES` with a CON-based save DC — and deliberately contributes NO damage term to that card's total, since its 2d6 lands only on the swing where you spend the use; **Frost's, Hill's and Storm's are ACTIVE EFFECTS** (Frost's +2 reaches the numbers; Hill's BPS resistance reaches the Defenses card; Storm adds the Prophetic State reaction) — Frost changes numbers the sheet already prints, so its card carries the start/End toggle and its +2 reaches the STR/CON saves and Athletics — a bare Use counter left the bonus living nowhere. A rune opts in with `channel.activeEffect`; the one-shot runes carry no key and are unchanged), and **5 of the 6 rune passives**: skill advantage (Cloud/Frost/Stone/Storm) through the new `skills/skillAdvantage.js` — the `SKILL_ADVANTAGE_SOURCES` extraction that had been flagged, which absorbed the Champion hardcode out of `CharacterDetail` — and Hill Rune's poison resistance (`defenses.js`) + advantage on saves vs. poisoned (`saveFeatures.js`). Both of those tables gained an optional **`applies(ctx)`** predicate, the escape hatch for a gate the declarative class/subclass/level keys can't express (a rune depends on `character_data`, not on level).
  **Stone's darkvision 120 ft is now real** — QA asked where the sheet states the range and the answer was nowhere, so it drove the `senses/senses.js` module (see the Senses bullet above) and is a `passive.vision` data row. Deliberately still prose, each because the app has NO model to hang it on: the **Fire Rune's +PB to tool checks** (the Tools sub-tab lists tool names — there is no computed tool-check number anywhere) and **Storm's can't-be-surprised** (no surprise/initiative state). Each is shown as the rune's passive text on the item card.
  Still to build: **Bonus Proficiencies** (smith's tools + Giant — a FIXED grant, which has no mechanism yet; planned as a derived `classData/subclassProficiencies.js`, the sibling of `subclassSpells.js`, since there is no pick to store). **Deliberate scope limit on rune-bearing objects:** RAW a rune may be inscribed on "a weapon, a suit of armor, a shield, a piece of jewelry, or something else you can wear or hold in a hand", but the assignment will offer **weapons, armor and shields only** — the only wearable/holdable things the inventory models as equippable. **Revisit jewelry and other worn objects when items/armor get more robust** (they live in `adventuring_gear`/`magic_items` today with no worn state).
- **Active effects (a feature you switch ON and OFF) — mechanism built, 4 entries authored** (Giant's Might, Channel Rune Frost/Hill/Storm). **RULE (user): anything named "Channel …" (Channel Rune, Channel Divinity) gets an active status, and while active it must DO something** — Hill's `resistances` grant lands on the Defenses card (via `activeEffectResistances` in defenses.js), Storm's prophetic state adds a `hidden`-gated **Prophetic State** reaction card that exists only while it runs; a bare Use counter that spends a charge and changes nothing is a bug (QA). Channels that are instantaneous or land only on the target (Cloud/Fire/Stone) have no self-state yet (`effects/activeEffects.js` + `ActiveEffectsBanner`). **This deliberately REVERSES the app's long-standing "no active-effect model" line** — the line that still keeps Rage damage, Divine Smite and spell-granted magic weapons as prose, because a bare "+2d6" on a card would be false most of the time. Rune Knight **Giant's Might** forced it: the feature changes the character's SIZE, grants advantage on two roll types and adds a scaling damage die, which is far too much to leave as a paragraph the reader applies by hand. State is `character_data.active_effects: string[]`; a definition is a data entry gated by class/subclass/level/edition, whose `grants(level)` returns only the fields it changes (`size`, `advantageAbilities`, `advantageSaves`, `attackDie`, `reachBonus`, `checkBonus`/`saveBonus` — each `{abilities, amount}`) and whose `summary(level)` is its one-line description — so no surface retypes a number the definition owns. **Every grant must have a consumer, or the effect only pretends to do something**: QA found Giant's Might switched on, the banner announcing advantage on Strength checks and saves, and the Abilities & Skills panel completely unchanged — `advantageAbilities`/`advantageSaves` were produced and read by nobody. They are now wired to the two tables that already own those questions: `skills/skillAdvantage.js` gained an `active-effects` source (an ABILITY fans out to every skill keyed off it via the shared `SKILL_MAP`/`skillsForAbility` in `skillMath.js`, since RAW is "Strength checks", not "Athletics"), and `skills/saveFeatures.js` gained the Giant's Might entry with `advantageAbilities`. Check the same when adding an effect: a `grants` field with no reader is a silent no-op. **Duration is deliberately NOT modelled** (nothing tracks rounds): an effect runs until switched off, and the stated duration is text. **Spending an effect's charge ALWAYS switches the effect on, in ONE patch** — a counter that went down without the effect coming on is a use the player paid for and did not get. That rule now lives in the shared **`RestResourceControl`**, not in one caller: it takes an `activeEffect` (derived by the tracker from **`effectForResourceKey(row.key)`**, so no config edit can forget a row) and, while the effect runs, offers **End** instead of Use — ending never refunds. QA found the Action Economy card doing this correctly while the IDENTICAL row on the sheet's rest tracker was a plain counter, leaving `channel_rune_frost_used: 1` with `active_effects: []`. Two spend points for one charge with only one of them starting the effect is exactly what the single-patch rule exists to prevent, so put the rule in the control, not in the surface. The Action Economy card keeps its own richer toggle and suppresses the plain Use. A second always-visible `ActiveEffectsBanner` above the tabs was tried and **removed at the user's call** — a standing strip restating the effect's numbers over every tab was noise next to the card that already says it. Do not re-add it (preserved-intent rule). What a running effect changes is instead shown by the surface that OWNS each number: the skill/save `adv` tags (`skillAdvantage.js` / `saveFeatures.js`), the attack card's damage total, and the size that `creatureSize` resolves. **Known gap left by the removal:** creature **size** now has no display of its own again — it is computed (Heavy-weapon rule, `creatureSize` with the effect context) but nothing prints it, so growing Large is invisible on the sheet. Put it where size actually matters when that's worth fixing, not in a banner. `creatureSize(characterData, race, ctx)` now takes an optional effect context and resolves the grown size at that ONE function every consumer already calls, so the Items and Action Economy tabs can't disagree. A long rest resets the *charge* but deliberately does NOT switch the effect off (a 1-minute effect ended long before the rest; a rest clearing it would imply a duration the app doesn't model). **The second effect proved the model and extended it exactly as intended** — Rune Knight **Channel Rune: Frost** (10 minutes of +2 to all Strength/Constitution ability checks and saving throws) was a data entry plus two `grants` fields, no sibling table: `checkBonus`/`saveBonus` `{abilities, amount}`, because Giant's Might changes what you ROLL while Frost changes a NUMBER the sheet already prints. Abilities are named rather than skills (RAW is "checks that use Strength or Constitution"), so each consumer maps the ability to what it shows. It also added the `applies(ctx)` gate to `getActiveEffectDefs` (the defenses.js/saveFeatures.js escape hatch) — a rune effect is only offered while the rune is carved onto an EQUIPPED object, and numeric bonuses SUM across effects where advantage does not. Consumers: `activeEffectCheckParts(ability, ctx)` / `activeEffectSaveParts(ability, ctx)` return breakdown-ready `{key,label,value}` terms fed into `skillMath`'s new **`extras`** parameter by the skill rows and the Saving Throws grid, so a raised number always names its source in the click-to-see-the-math panel. **A moved number is not enough on its own** (a +7 Athletics with nothing beside it reads as the sheet's ordinary total), but **the marker must never be a second number**: a `+2` tag beside the total was tried in QA and read as "+7, and +2 on top of that" — the bonus is already inside the +7. So the affected row **colours its TOTAL indigo** instead (the existing `skill-bonus-{skill}` / `save-bonus-{ability}` values, no extra element), and each surface names the effect in words: the skills legend gains "Indigo total = includes {sources}", and the saves grid — which has no legend — gets a `saves-effect-note-{key}` line phrased as inclusion, "STR & CON saving throws include +2 from Channel Rune: Frost", never "+2 to …", for the same reason. Both come from `activeEffectCheckSources`/`activeEffectSaveSources` (grouped by source) and `activeEffectCheckBonus`/`activeEffectSaveBonus` (the per-ability total). Adding Rage/Hex/Hunter's Mark should be the same shape — check whether `grants` needs another field, not whether it needs a sibling table.
- **Senses (vision type + range) — BUILT** (`senses/senses.js` + `SensesPanel`, a **Senses** card in the CharacterDetail **Stats -> HP & Movement** sub-tab). Answers "how far can I see in the dark?", which the sheet could not: darkvision was granted in three places and displayed in NONE — the racial `Darkvision` trait, the Drow's `Superior Darkvision`, and the Rune Knight's Stone Rune all carried it as prose inside a description, so a player had to read three blurbs and work out which range won (found in QA on the Stone Rune). Wired CENTRALLY in CharacterDetail like the Defenses card, so hand-written sheets get it too, and gated on `hasSenses(ctx)` so ordinary vision costs no card. **Ranges SUPERSEDE, they do not stack** — a Drow Rune Knight sees 120 ft, not 180 — so `getSenses` keeps the largest per sense and returns the losers as `superseded` rather than dropping them, because a player who stows the rune-bearing axe needs to know they fall back to 60 ft rather than to nothing. Sources are the same `resolve(ctx)`-per-entry shape as `skillAdvantage.js` (race traits gate declaratively; the rune gates on `activeRunes()`, so unequipping removes the 120 ft). Sight-affecting traits that grant no radius (**Sunlight Sensitivity**) come back from `getSightNotes` and render name-only on the shared `FeatureNote` with their text read out of `RACE_TRAIT_DESCRIPTIONS`. Adding a source is a data row: the Stone Rune's is `passive.vision` in `runesData`, a race's is a `RACE_SENSE_TRAITS` entry. Deliberately NOT in scope: light sources (a torch is equipment), the blinded condition (a Conditions panel is the follow-up), blindsight/truesight/tremorsense (nothing in the app grants them yet — they are one row each when something does), and what a creature can see of YOU.
- **Damage defenses (resistances / immunities / flat reductions) — BUILT** (`defenses/defenses.js` + `DefensesPanel`, rendered as a **Defenses** card in the CharacterDetail **Stats → HP & Movement** sub-tab). Answers "is this fire damage halved?", which the sheet previously could not — every resistance in the app was prose inside a feature description. Wired CENTRALLY in CharacterDetail rather than through a sheet slot, so it works for data-driven **and** hand-written sheets alike (unlike the `afterHpNode` racial tracker); the card is gated on `hasDefenses(ctx)` so a character with none gets no empty card. **The rule the whole module is shaped around: the app has NO active-effect model** (no `isRaging`, no concentration, no durations — deliberate; same line companions/Encounters-V1/spell-granted-magic-weapons draw), so every entry is either `condition: null` (**Always on**) or carries the condition as visible text (**Situational**), and the two groups are never flattened. A bare "Bludgeoning" row for a Barbarian would be false most of the time. Rows are `{typeLabel, valueLabel, qualifier, condition}` — a `qualifier` narrows *which* damage of that type is covered ("from spells", "from nonmagical weapons", "except force"), because several RAW defenses key on the damage's SOURCE not its type; a `typeLabel` override names an unpersisted player choice ("Your choice" for the Fiend's rest-chosen type, "By patron" for the Genie) instead of inventing a type or dropping the row. Damage types may be a **function** of the character for stored choices (Dragonborn `draconic_ancestry.damage`, Sorcerer `draconic_bloodline.damage`). Feature rules text is read back out of `SUBCLASS_DATA`/`CLASS_FEATURES_*`/`RACE_TRAIT_DESCRIPTIONS`, so it can't drift; the name expands via the shared `FeatureNote` (NOT a copy of `SaveFeaturesPanel`, which hand-rolls its own collapse — that one is a missed reuse worth retro-fitting). **37 entries authored** across race/class/subclass. **FEATS come through the structured `damage_reduction` feat effect instead** (`featEffects.getFeatDamageReductions`, backend `seed_feats.py` `_damage_reduction()`; `amount` accepts the `'pb'` sentinel — 2024 Heavy Armor Master scales with PB and drops 2014's "nonmagical" clause), so a feat keeps ONE mechanization route. Deliberately EXCLUDED, each for a stated reason: reaction damage reduction (Deflect Missiles, Slow Fall, Spirit Shield, Song of Defense, Warding Maneuver → belongs on Action Economy), features that let you OVERCOME resistance (Ki-Empowered Strikes, Magic Arrow → `weaponMagic.js`), defenses granted only to OTHERS (Ancestral Protectors, Expansive Bond, Smite of Protection), condition-only immunity (Beguiling Defenses, Fungal Body, Divine Health — a different axis from damage; a **Conditions** panel is the follow-up), a summoned companion's own defenses (the Echo Knight echo's condition immunity → companion panel), and choices the app never persists (Totem Spirit's Bear, Elemental Epitome, Transmuter's Stone). **Feat `effects` are SNAPSHOTTED onto `character_data.feats[i]` at acquisition** (`FeatsSubTab`); re-running `seed_feats.py` now refreshes those snapshots (`sync_character_feat_snapshots`), so a new `/feat-effects` pass reaches existing characters after a re-seed.
- Features affecting the character's OWN saving throws — **mechanism built, 3 entries authored**. `skills/saveFeatures.js` + `SaveFeaturesPanel` list them under the Saving Throws grid (name only, click for the rules text), pulling descriptions out of the feature tables so nothing is retyped. Authored: Cavalier **Born to the Saddle**, Rune Knight **Hill Rune** (vs. poison, `applies`-gated on the rune being carved + equipped) and Rune Knight **Giant's Might**. An entry may now carry **`advantageAbilities: ['strength']`**, which puts an `adv` tag ON the save row itself (`save-advantage-{ability}`, teal, mirroring the skill grid's) plus an "Advantage — {feature}" line in that save's breakdown — `saveAdvantageSourcesFor(ability, ctx)` / `getSaveAdvantageAbilities(ctx)`. It is for advantage scoped by ABILITY only: advantage scoped by SITUATION (Born to the Saddle — falling off a mount; Hill Rune — against poison) deliberately omits the field and stays panel-only, since a tag on a save row asserts "roll this twice" with no room for the condition that makes it true. Giant's Might is also the first entry gated on an **active effect** rather than on what the character permanently has (`applies: isEffectActive(…, 'giants_might')`), so the tag appears exactly while the effect is switched on. Every further feature is a one-line data entry; the un-authored candidates are the ones the grid can't express as a number — Paladin Aura of Protection (+CHA), Fighter Indomitable (reroll), Monk Diamond Soul / 2024 Disciplined Survivor, Rogue Slippery Mind, Ranger Iron Mind, Barbarian Rage (STR) + Danger Sense (DEX), Bard Countercharm, Artificer Soul of Artifice, Sorcerer Tides of Chaos / Psychic Defenses, Cleric Corona of Light, Fighter/Cavalier Elegant Courtier, Champion Survivor (+3 death saves). Race traits (Gnome Cunning, Dwarven Resilience, Stout Resilience) and feats (Lucky) are the same shape — an entry gains a `race`/`feat` key plus one line in `matches`, with no panel change. **Scope rule:** only your OWN saves; a feature that gives an ENEMY disadvantage on ITS save (Eldritch Strike) stays on the surface where it fires.
- Subclass grants (proficiency + class-pool picks) — **one consolidated mechanism, proven on four subclasses**: `subclassGrants.js` handles a subclass *proficiency* choice (Battle Master "Student of War"), a *class-pool* pick (Champion "Additional Fighting Style"), a *skill + cantrip* pair (Arcane Archer "Arcane Archer Lore"), and a **split-destination** choice (Cavalier "Bonus Proficiency" — a skill **or** a language) via the LevelUpWizard `subclass-grants` step. Split destinations work through an optional **per-option `storeField`** that overrides the grant's default, resolved by `grantStoreFieldFor`/`grantStoreFields` and honoured by `applyGrant` (which groups picks per destination) — so one picker can write a skill to `skill_proficiencies` and a language to `subclass_languages`; Samurai's identical Bonus Proficiency is a pure data entry on top of it. `subclass_languages` surfaces as the CharacterDetail languages panel's **"From Subclass"** group (`languages-from-subclass`) — before that it was stored and deduped against but never displayed. `surface` says which panel displays the chosen value and now has four values — `'sheet'` (the ClassSheet subclass block, owed-slot fillable; the default and the ONLY one that renders there), `'banner'` (Items-tab proficiency banners), `'skills'` (the Abilities & Skills panel), `'spells'` (the Spells tab's Subclass source) — so a grant is never shown twice. Merged from the former `subclassProficiencyData` + `subclassLevelChoices` while only two subclasses were wired (cheap consolidation window). Adding another subclass (either kind) is pure data entry. Kept SEPARATE: `levelChoicesData.js` (class-scoped pools — Metamagic/Invocations — with cumulative knownAtLevel + replace-on-level-up + minLevel; a genuinely different shape, and the Sorcerer/Warlock sheets' pool source). Don't fold class-scoped pools into subclassGrants; if a 3rd distinct shape appears, reconsider — flagged via the `/subclass-features` skill.
- **Subclass-GRANTED spells (fixed, not chosen) — mechanism built, 1 entry authored** (`classData/subclassSpells.js`). A subclass feature that hands a character a specific spell outright, with no pick to store. This is the OPPOSITE shape from `subclass_cantrips` (subclassGrants `surface:'spells'`) on every axis: that one is CHOSEN from a pool at level-up and stored in character_data; this one is FIXED, may be LEVELED rather than a cantrip, and is gained automatically at a set level — so it is **derived** from class+subclass+level at render time (the `raceCantrips.js` pattern), never written into character_data. `SUBCLASS_SPELL_GRANTS[class][subclass] = [{spell, level, minLevel, feature, ability, atWill, note}]` → `getSubclassGrantedSpells({charClass, subclass, level, edition})` returns `{cantrips, leveled}`. Entries are edition-neutral unless a revision actually moves something. Authored: **Psi Warrior → Telekinesis** (L18 Telekinetic Master, at will, Intelligence). Both kinds feed the ONE Spells-tab **Subclass** source (`subclass-spells`, retitled from "{subclass} Cantrips" now that it holds leveled spells), so a character never sees two subclass spell cards; a granted spell's row carries an `subclass-spell-grant-{name}` badge naming the ability, because a Fighter has no spellcasting ability anywhere else on the sheet to infer INT from. It is also enough on its own to give a non-caster the Spells tab. **Deliberately NOT a rest resource**: an at-will spell has no counter, so there is nothing for the backend rest flow to reset — a subclass spell with limited uses belongs in a rest-resource table beside its counter (the `racialRestResources.js` shape), not here.
...
- **Encounters / Roll Initiative — V1 built (GM-driven), worklist in `docs/tickets/roll-initiative-v1.md`.** Done: the shared `initiativeData.js` helper (one source for the modifier — DEX + feat `stat_mod`s + the Champion advantage flag — consumed by both the Stats tab and the encounter page), the persisted encounters module, `rest_type:'initiative'` + the `_INITIATIVE_RESOURCES` table, and the page. **All 9 initiative-triggered features in the app are authored** across the three shapes — `regain_when_empty` (Ever-Ready Shot, Battle Master Relentless ×2 editions, Bard Superior Inspiration, Monk Perfect Self which regains 4, Samurai Tireless Spirit, Echo Knight Legion of One), `floor` (Monk 2024 Perfect Focus), and `opt_in` (Monk 2024 Uncanny Metabolism, which spends its own charge and so is offered as a checkbox on the page via the initiative-options endpoint). Building Tireless Spirit also **created the Samurai Fighting Spirit pool**, which had never existed (a `subclass:'Samurai'` rest-resource row on the Fighter config + its long-rest reset). Deferred by design: monsters (waiting on the Bestiary tab), turn/round tracking, and players rolling on their own screens (there is no live-update transport in the app — no websockets, no polling).
- **Character personality (traits / ideals / bonds / flaws) — NOT BUILT.** The same gap the Appearance section just closed for physical description: NPCs have `personality_traits`, `ideals`, `bonds` and `flaws` columns; player characters have none, even though backgrounds carry a `characteristics` JSON the creation flow never turns into stored character state. The shape is already proven — a sibling JSONB column plus a catalog module beside `appearance/appearanceData.js`, rendered as a second Narrative-tab card — and the background's suggested traits would make good datalist suggestions. Deliberately left out of the appearance pass to keep its scope honest.
- Loot table UI
...
- **Game-mechanics reference pages — framework + Jump built (vertical slice).** Pattern: a pure helper module (`jumpData.js`) is the single source of truth, consumed by both a static encyclopedia page (`JumpPage`, routed under the Mechanics tab via `mechanicsRegistry.js`) AND a computed surface on the relevant sheet (Stats-tab `JumpCard`) that links back to the page. Built: Jump (Flow A — helper+card), Armor Class (Flow B — reuses computeArmorClass, linked from the Items-tab AC summary), Action Economy (Flow B — reuses actionEconomyData with an edition toggle, linked from the Action Economy tab), Hit Dice (Flow B — documents HitDiceTracker + rest recovery, linked from the Stats-tab Hit Points & Movement card), Loading (Flow B — reuses inventoryData's weaponLoadingNote, edition toggle since 2024 dropped the property, linked from the Items + Action Economy tab loading notes), Object Interaction / "Drawing & Stowing Weapons" (page-only, NO helper/card — a pure rules concept with nothing to compute; no edition toggle; linked from the Dual Wielder feat in both feat surfaces + the Items-tab Weapons sub-tab), Spacing / "the 5-Foot Rule" (page-only, NO helper/card — no edition toggle; the 5-ft rule, opportunity attacks + Disengage, within-5-ft disadvantage on ranged weapon AND ranged spell attacks, plus a page-local list of the in-app feats/features that bend it; linked from each ranged weapon row in the Items tab, the CharacterDetail Spells tab, and the Action Economy tab (per ranged weapon entry + the Spell section)). **Special Melee Attacks** (Flow B — Grapple and Shove. The helper `combat/specialAttacksData.js` is NEW and is the source of truth for BOTH surfaces: `oneSizeLarger`/`sizeLimitNote` for the size limit, per-edition `SPECIAL_MELEE_ATTACKS_*` for the rules text, and `specialAttackEntries({size, edition, handFree})` which builds the Action Economy cards. Carries an **edition toggle** — 2014 resolves the attempt with a contested Athletics check and 2024 with a save against your Unarmed Strike DC, a real difference rather than a rewording). **Magical Attacks & Resistance** (Flow B, page-only — the per-weapon answer already lives in `weaponMagic.js`: resistance to nonmagical B/P/S, the three routes to a magical attack, the in-app features granting it, a worked example run through `magicalAttackSource`, the 2024 statblock change as prose rather than an edition toggle, and what the app can't track; linked from INSIDE the `MagicAttackBadge` expanded note so the link exists only where the Magic tag does). **Taking Damage** (Flow B — `defenses/defenses.js` gained the exported **`applyDamage({amount, resistant, vulnerable, reduction})`** returning `{final, steps}`, which encodes the RAW order (vulnerability doubles → resistance halves, rounding down → flat reduction subtracts) and the fact that resistance/vulnerability are BOOLEANS not counts (neither stacks; both together cancel). It is currently consumed by the PAGE rather than the sheet — the app displays defenses instead of resolving hits — but it lives in the helper so the worked examples are executed rather than typed, and so it is the obvious home if damage resolution is ever built. Page covers the four mitigations, the order with the common wrong answer shown for contrast, rounding, temporary hit points + 7 real in-app sources (marked with WHO each grants to — Rally targets an ally, not you), reaction-based reductions listed separately with why the Defenses card excludes them, damage-vs-condition immunity, and the honest limits — notably that NOTHING in this app gives a character vulnerability (it exists only outward-facing: a Cleric feature imposes it, Ranger Hunter's Sense reveals it). No edition toggle; cross-links Magical Attacks rather than restating "nonmagical"; linked from the Defenses card bottom via `defenses-learn-more`). Registry roadmap ("Coming soon" cards): Conditions, Concentration — **Conditions is now owed by two pages**, since Special Melee Attacks describes Grappled and Prone only as far as a grapple card needs. **Use the `/mechanics-page` skill** to author each — it encodes both flows (Flow A new mechanic: helper+card → page → link; Flow B page-only when the math already exists like AC/Action Economy: page → link from the existing surface).
...
├── test_characters.py              # character CRUD + visibility, TestGmNotes (gm_can_set/get, stripped_from_owner, player_cannot_set), TestGmDelete (gm_can_delete, other_player_cannot), TestCampaignEdition (defaults_to_5e, create/update/list), TestCharacterListFieldRoundTrip incl. character_data, TestExperiencePointCap (7 — an over-award clamps to 355,000, exactly-at and below the cap untouched, the reported level-19 over-award case, negative XP floored at 0, the clamp is stored not just displayed, an update omitting XP leaves it alone), TestApplyRest (GM can short/long rest, player 403, non-member 403, cross-campaign filter, Warlock pact slots, Wizard HP+slots, hit dice recovery (half total rounded down min 1 — even + odd level), response shape, Dragonborn Breath Weapon short-rest reset, Relentless Endurance long-rest-only, Drow darkness level-gated, Portent cleared on long rest, Portent untouched on short rest, Battle Master Superiority Dice reset on short + long rest, non-Battle-Master Fighter unaffected, feat resource pools — Lucky+Martial Adept reset on long rest, only short-recharge feat resource resets on a short rest), TestCharacterMusic (owner/GM upload, non-owner player 403, disallowed extension 400, delete clears url), **TestCharacterAppearance** (10 — the physical-description JSONB: null by default, EVERY one of the 23 catalog keys round-trips (multi-line and quoted text included), settable at CREATION (the create path silently dropped it until this test caught it), owner and GM can both write it, a write REPLACES the object so a cleared field really clears, an update that omits it leaves it alone (saving another section must not wipe a description), an arbitrary field set is accepted since the catalog lives in the frontend and is meant to grow, it can be cleared to {}, and another player in the campaign can READ it once the character is visible — unlike gm_notes/personal_notes it is public to the party, being what the other characters see just by looking), feat spell-grant free cast (long rest resets feat_freecast_<slug>_used, short rest leaves it), Eldritch Knight spell slots (long rest resets spell_slots, short rest doesn't, Champion Fighter untouched), **TestInitiativeRest** (64 — `rest_type:"initiative"`: GM applies, player 403, non-member 403, cross-campaign filter, unknown rest_type 422; Ever-Ready Shot regains 1 only when the pool is EMPTY, never exceeds it, level-gated, other subclass/class untouched; it is not a rest — Second Wind/Action Surge/HP/spell slots left alone; Battle Master Relentless in BOTH editions incl. **the Martial Adept 7th die** (6 spent is not empty for that character — the case a flat pool size would get wrong); Bard Superior Inspiration with the pool following CHA (empty at CHA 18, not at CHA 20, minimum of one); Monk Perfect Self regaining **4**; Perfect Focus's `floor` shape (tops up to PB, no-op at or above it, **never reduces** a fuller pool, 2024-only, level-gated); Uncanny Metabolism's `opt_in` shape (does nothing unchosen, regains PB when chosen, never overfills, refuses once its charge is spent, doesn't burn the charge on a full pool, charge returns on a long rest, applies only to the character who chose); the initiative-options endpoint (lists an available opt-in, marks a spent one unavailable, omits characters with no choice, level-gated, player 403, cross-campaign filter, empty id list); Samurai Fighting Spirit (resets on a long rest, survives a short one, other subclasses untouched) + Tireless Spirit; and one call applying all of them across a party); Cavalier pools (both reset on a long rest with named changes, both survive a short one, Warding Maneuver neither reset nor promised below L7, another subclass untouched), Echo Knight pools (all three reset on a long rest with named changes, a L5 Echo Knight is promised only Unleash Incarnation, a short rest recovers Shadow Martyr ALONE and leaves the long-rest two, Shadow Martyr untouched below L10, another subclass untouched) + Legion of One (regains a use when empty, does nothing with a use left, **sizes "empty" by Constitution** — 3 spent is empty at CON 16, level-gated at 18, and is not a rest), Rune Knight pools (both reset on a long rest with named changes, a L5 is promised only Giant's Might, both survive a short rest, a long rest does NOT switch off a running Giant's Might, and another subclass is untouched), Psi Warrior pools (all five reset on a long rest with named changes, a L5 is promised only the Psionic Energy dice, and a short rest recovers the two once-per-rest CHARGES — the die regain + Telekinetic Movement — while leaving the POOL and both long-rest free uses alone, which is the split the subclass exists to test; another subclass untouched), Rune Knight Channel Rune (resets on a SHORT rest with a named change — the only Rune Knight pool that does — resets on a long rest alongside Giant's Might, a short rest returns it while leaving both long-rest pools spent, every one of the six counters is zeroed rather than only the carved ones (which runes are carved is frontend state, so a rune moved between rests can't keep a stale count), and another subclass is untouched) (157 tests)
...
├── test_report_feat_effects.py    # the CLAUSE-level feat coverage report (no DB): note statuses (pending/surfaced/unmodelable), feat status (one wired effect no longer hides a pending clause; complete only when every note is surfaced or signed off), counts, the ratchet (a new pending note FAILS, losing a complete feat fails, building a clause is an improvement) + per-note authoring guards over both effect tables (at most one status tag; a tag must say where / what model)
├── test_feat_seed_data.py         # authored-feat GUARDS (no DB): a feat whose text grants a reaction authors a reaction action; an Attack-action bonus action authors the combo trigger; guard self-tests; Shield Master's Interpose Shield in both editions + sync_character_feat_snapshots (refreshes a stale snapshot keeping choices, no-op when current) (8 tests)
...
│   └── pages/
│       ├── CharacterList.jsx
│       ├── CharacterList.test.jsx    # loading, fetch with campaignId, error, empty state, card render, click-to-navigate, GM title+visibility toggles+reload, player title+no toggles; rest buttons (show, disabled/enabled on selection, checkboxes, select all, dialog opens for short/long, character names shown, confirms short/long rest calls applyRest, cancel no-op); rest buttons hidden for players + GM player-view; long-rest summary lists a feat spell free cast (Magic Initiate → "Mage Armor (feat free cast)"); long-rest summary lists "All spell slots" for an Eldritch Knight Fighter; both rest summaries list "Arcane Shot" for an Arcane Archer; long-rest summary lists Unwavering Mark for a L5 Cavalier but adds Warding Maneuver only at L7, and lists Fighting Spirit for a Samurai; Echo Knight rest summaries (a L5 gets Unleash Incarnation alone on a long rest, a L15 gets all three, a short rest lists Shadow Martyr ALONE and only from L10); Psi Warrior rest summaries (a long rest lists the Psionic Energy dice, adding Psi-Powered Leap + Bulwark of Force only at L15, while a short rest lists the two once-per-rest charges and never the dice — a summary that flattened the two would promise the pool back after a nap); Rune Knight rest summaries (a long rest lists Giant's Might, adding Runic Shield only from L7, while a short rest promises neither — both pools are long-rest only) (43 tests)
│       ├── CharacterCreate.jsx
