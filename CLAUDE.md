...
- **CharacterDetail — tab layout:** six-tab structure for spellcasters, five-tab for non-spellcasters (shadcn/ui Tabs); default tab is "narrative":
  ...
  - **Tab 1 "Stats"** — split into **three button sub-tabs** (`stats-subtab-identity` / `stats-subtab-abilities` / `stats-subtab-hp`, default identity; same pattern as the Features tab toggle):
    ...
    - **"Abilities & Skills" sub-tab** — one "Abilities, Saves & Skills" card: ability score grid (read-only divs — scores change only via level-up ASIs), derived stats row (Prof Bonus / Initiative / Passive Perception incl. feat `stat_mod` notes), saving throws (toggles are `savingThrows` state — this card owns their Save via `saveClassData`, which now also commits `savingThrows`; the Features-tab card no longer tracks saving-throw dirtiness), and the 18-skill `SkillsDisplay` panel
    - **"HP & Movement" sub-tab** — Hit Points & Movement card (class sheet with `section="stats"`, receives a full `scores` prop so Barbarian/Monk unarmored-AC boxes compute correctly; the **Max HP value itself shows the effective total** via **`MaxHpValue`** — the CON-independent roll base (`hpRollBase`) + CON×level + passive bonuses, with a small "+N Source" note for the passives only (e.g. "+1 Draconic Resilience"; CON is part of the base number), never mutates stored HP — passed in as the sheet's `maxHpNode` slot (with `conMod` from the identity draft, so editing CON updates Max HP live). The HitDiceTracker heal cap uses `effectiveMaxHp` computed the same way. **No Armor Class field here** — AC lives only in the Items tab (computed from equipped armor); the Stats combat block is HP / Hit Dice / Speed only. The **feat speed annotation** (`speed-feat-note`, hand-written sheets only) renders inside this card, as do the **Relentless Endurance note** (`relentless-endurance-note`, via raceCombatNotes, for a Half-Orc) and the **Survivor note** (`survivor-note`, via subclassCombatNotes, for a L18 Champion Fighter — the start-of-turn 5+CON self-heal reminder, +3 death saves in 2024). The **Relentless Endurance use-tracker** (`RacialResourceTracker` filtered to `HP_ADJACENT_RACIAL_KEYS`) also lives here — data-driven sheets render it **between Max HP and Hit Dice** via the CombatBlock `afterHpNode` slot (passed through ClassSheet); hand-written sheets show it right below their combat block until they migrate (same interim pattern as `speed-feat-note`)) + **Jumping card** (`JumpCard` — computed long/high jump distances from STR + Athlete feat, expandable formulas, "Learn more" → /encyclopedia/mechanics/jump; display-only)
