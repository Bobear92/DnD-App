...
| POST | /api/characters/campaign/{id}/rest | Yes (GM of campaign); body: `{ rest_type: "short"\|"long", character_ids: int[] }`; returns `RestResponse` with per-character change summaries |
...
│   │   ├── WizardSheet.jsx      # Wizard (5e): full caster, spellbook, arcane recovery; Spells tab: "Prepared" sub-tab (Arcane Recovery "Use (Short Rest)" button disabled when no expended slots + slot tracker (− for all, + GM-only) + Cast button per prepared spell + cantrips) + "Prepare Spells" sub-tab (spellbook chips + lock/unlock + Other Prepared + encyclopedia link) ✅
...
│   │       ├── WizardSheet.jsx    # Wizard (2024): memorize spell L1, arcane recovery; Spells tab: "Prepared" sub-tab (Arcane Recovery "Use (Short Rest)" button disabled when no expended slots + slot tracker (− for all, + GM-only) + Cast button per prepared spell + cantrips) + "Prepare Spells" sub-tab (spellbook chips + lock/unlock + Other Prepared + encyclopedia link) ✅
...
- **`getRestSummary(cls, edition, level, restType)`:** client-side helper (mirroring backend logic) that returns a `string[]` describing what resources reset; used only to populate the pre-confirm dialog — the backend is the source of truth for what actually changes
...
- **Spell slot tracker (Wizard 5e + 2024 Prepared sub-tab):** `−` button (use a slot) shown for all non-readOnly users; `+` button (restore a slot) shown only for GMs (`isGm && !readOnly`) — players cannot manually restore slots; Arcane Recovery "Use (Short Rest)" button disabled when no slots are expended (enabled only when `someSlotExpended`); clicking when "Used" resets it (long rest)
