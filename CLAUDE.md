...
│   │   #   classData/   → classChoicesData, classFeatures5e, classFeatures2024, classProficienciesData, classProgressionTables, maneuversData, levelChoicesData, subclassGrants, ClassOverview, subclassData/
...
- **CharacterDetail — tab layout:** six-tab structure for spellcasters, five-tab for non-spellcasters (shadcn/ui Tabs); default tab is "narrative":
  ...
  - `hasSpells = SPELLCASTING_CLASSES.has(char_class) || raceGrantedCantrips.length > 0 || hasFeatSpells` (`hasFeatSpells` = getFeatGrantedSpells(feats) has any cantrip/leveled spell **or ritual book** — so a non-caster Fighter with Magic Initiate **or Ritual Caster** gets a Spells tab; the inner Feats spell-source sub-tab uses the same cantrip+leveled+ritualBook test, matching FeatSpellsSection's render gate)
