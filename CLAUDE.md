...
- **CharacterDetail — tab layout:** six-tab structure for spellcasters, five-tab for non-spellcasters (shadcn/ui Tabs); default tab is "narrative":
  ...
  - **Tab 1 "Stats"** — Identity & Ability Scores card (the Race/Species block shows a "Draconic Ancestry" line/badge when `character_data.draconic_bloodline` is set) + Hit Points & Movement card (class sheet with `section="stats"`, receives a full `scores` prop so Barbarian/Monk unarmored-AC boxes compute correctly; the **Max HP value itself shows the effective total** via **`MaxHpValue`** — the CON-independent roll base (`hpRollBase`) + CON×level + passive bonuses, with a small "+N Source" note for the passives only (e.g. "+1 Draconic Resilience"; CON is part of the base number), never mutates stored HP — passed in as the sheet's `maxHpNode` slot (with `conMod` from the identity draft, so editing CON updates Max HP live). The HitDiceTracker heal cap uses `effectiveMaxHp` computed the same way. **No Armor Class field here** — AC lives only in the Items tab (computed from equipped armor); the Stats combat block is HP / Hit Dice / Speed only; a **Relentless Endurance note** (`relentless-endurance-note`, via raceCombatNotes) renders under the sheet for a Half-Orc, and a **Survivor note** (`survivor-note`, via subclassCombatNotes) renders for a L18 Champion Fighter — the start-of-turn 5+CON self-heal reminder, +3 death saves in 2024) + **Inspiration card** (`InspirationCard` — a numeric counter defaulting to 0 with − / + steppers, persists immediately via `autoSaveClassPatch` to `character_data.inspiration`; surfaces the 2024 Champion **Heroic Warrior** note `heroic-warrior-note` via subclassCombatNotes) + **Jumping card** (`JumpCard` — computed long/high jump distances from STR + Athlete feat, expandable formulas, "Learn more" → /encyclopedia/mechanics/jump; display-only) + **Racial Features card** (`RacialResourceTracker` — ALL rest-rechargeable racial traits incl. the Dragonborn **Breath Weapon** (moved here from the old Weapons & Armor tab), Relentless Endurance, Drow Magic L3+, Infernal Legacy L3+; shown when the character has ≥1 such trait; shares `classSection`/`saveClassData` so Save/Reset persist with the rest of character_data)
