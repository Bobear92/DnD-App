...
//
// Each option resolves to { name, description, improvedDescription }. `improvedDescription`
// is the 18th-level upgrade ("Superior Arcane Shot" in the app's feature table) written as
// the WHOLE option text with its dice already raised — not a trailing "the damage increases
// to 4d6" clause. A clause appended after the description reads like a second, additional
// effect (players were adding 2d6 + 4d6), so the surfaces swap the paragraph instead of
// growing it. It stays a SEPARATE field so consumers can still level-gate it rather than
// baking a level tag into the text.
//
// The improved text is DERIVED, never authored twice: `improvedDice: [from, to]` raises every
// occurrence of a die in the description (Grasping Arrow upgrades two), and `improvedText`
// appends a sentence for the one option — Banishing Arrow — whose 18th-level effect adds
// damage where the base option deals none. Two copies of a paragraph would drift.
//
...
/** Resolve one authored option into { name, description, improvedDescription }. */
function withImprovedText({ improvedDice, improvedText, ...option }) {
  let text = option.description;
  if (improvedDice) text = text.split(improvedDice[0]).join(improvedDice[1]);
  if (improvedText) text = `${text} ${improvedText}`;
  return { ...option, improvedDescription: text };
}
...
export const ARCANE_SHOT_OPTIONS = [
  {
    ...
    improvedText: 'The arrow also deals 2d6 force damage to the target.',
  },
  {
  ...
  {
    // Both the poison and the slashing damage are 2d6, so the one substitution raises both.
    name: 'Grasping Arrow',
...
/**
 * The same DC in the shared click-to-see-the-math shape (`buildBreakdown`), so the number on
 * the Arcane Shot card expands into its arithmetic exactly like a skill or a saving throw.
 * The displayed value IS `total`, so the panel can't disagree with the number that opened it.
 */
export function arcaneShotSaveDcBreakdown(level = 1, intelligenceScore = 10) {
  const { pb, mod } = arcaneShotSaveDcParts(level, intelligenceScore);
  return buildBreakdown({
    parts: [
      { key: 'base', label: 'Base', value: 8, signed: false },
      { key: 'proficiency', label: 'Proficiency bonus', value: pb },
      { key: 'ability', label: 'INT modifier', value: mod },
    ],
  });
}
